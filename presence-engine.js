  ...
  class InteractionManager {
    ...
    receiveHighFive(from, fromNick) {
      ...
      if (this.pendingHighFives[from] && Date.now() - this.pendingHighFives[from] < 10000) {
        ...
        if (window.VDna) window.VDna.addXp(25);
    ...
    checkCoopUnlock() {
      ...
      if (hasBoth) {
        ...
        if (window.VDna) window.VDna.addXp(30);
        const body = document.getElementById('termBody');
        if (body) {
...
╚══════════════════════════════════════╝</span></div>`;
  ...
  class UIRenderer {
    ...
    init() {
      ...
      audioInd.className = 'mp-audio-ind';
      audioInd.title = 'Toggle spatial audio';
      audioInd.innerHTML = `<span class="mp-audio-ind-icon">🔇</span><span class="mp-audio-ind-bars"><span class="mp-audio-ind-bar"></span><span class="mp-audio-ind-bar"></span><span class="mp-audio-ind-bar"></span></span>`;
      ...
      this.barWrap.appendChild(audioInd);
    ...
    renderBar() {
      ...
      if (others > 0) {
        ...
        if (!this.engine.trophyAwarded) {
          ...
          if (typeof checkTrophy === 'function' && checkTrophy('networking_event') && typeof showTrophyToast === 'function') showTrophyToast('networking_event');
        }
      } else {
  ...
  class AudioSpatializer {
    ...
    _getMaster() { return window._spatialAudio && window._spatialAudio.getMaster ? window._spatialAudio.getMaster() : null; }
    _isEnabled() { return window._spatialAudio && window._spatialAudio.isEnabled(); }
