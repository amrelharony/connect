...
.term-chat-cursor { display: inline-block; width: 2px; height: 13px; background: #00e1ff; animation: termCursorBlink .8s step-end infinite; vertical-align: text-bottom; margin-left: 2px; }
@keyframes termCursorBlink { 50% { opacity: 0; } }
.term-chat-confidence { display: inline-block; font-family: 'JetBrains Mono', monospace; font-size: 7px; letter-spacing: 1px; text-transform: uppercase; padding: 2px 6px; border-radius: 4px; margin-bottom: 6px; }
.term-chat-confidence.high { background: rgba(34,197,94,.12); color: #22c55e; border: 1px solid rgba(34,197,94,.2); }
.term-chat-confidence.good { background: rgba(0,225,255,.08); color: #00e1ff; border: 1px solid rgba(0,225,255,.15); }
.term-chat-followups { margin-top: 8px; padding-top: 6px; border-top: 1px solid rgba(255,255,255,.04); }
...
/* LLM mode badges */
.term-chat-badge { display: inline-block; font-family: 'JetBrains Mono', monospace; font-size: 7px; letter-spacing: 1px; text-transform: uppercase; padding: 2px 8px; border-radius: 4px; margin-bottom: 6px; margin-right: 4px; vertical-align: middle; }
.term-chat-badge.ai { background: rgba(34,197,94,.12); color: #22c55e; border: 1px solid rgba(34,197,94,.2); }
.term-chat-badge.kb { background: rgba(139,148,158,.1); color: #8b949e; border: 1px solid rgba(139,148,158,.15); }
.term-chat-badge.loading { background: rgba(0,225,255,.08); color: #00e1ff; border: 1px solid rgba(0,225,255,.15); animation: llmPulse 1.5s ease-in-out infinite; }
...
/* Globe Data Viz */
.globe-hud { position:absolute; inset:0; pointer-events:none; z-index:10; font-family:'JetBrains Mono',monospace; display:flex; flex-direction:column; justify-content:space-between; padding:12px 16px; }
.globe-stat-row { display:flex; gap:20px; justify-content:center; padding-bottom:8px; }
.globe-stat { text-align:center; }
.globe-stat-val { font-size:16px; font-weight:700; color:#00e1ff; line-height:1; text-shadow:0 0 12px rgba(0,225,255,.4); }
.globe-stat-label { font-size:7px; letter-spacing:1.5px; text-transform:uppercase; color:rgba(255,255,255,.4); margin-top:2px; }
.globe-legend { display:flex; gap:12px; justify-content:center; align-items:center; font-size:7px; letter-spacing:.5px; color:rgba(255,255,255,.35); padding-bottom:20px; }
.globe-legend-dot { width:6px; height:6px; border-radius:50%; display:inline-block; margin-right:3px; vertical-align:middle; }
.gb-globe-btn { font-family:'JetBrains Mono',monospace; font-size:8px; letter-spacing:1.5px; text-transform:uppercase; padding:5px 14px; border-radius:100px; border:1px solid rgba(0,225,255,.2); background:rgba(0,225,255,.05); color:#00e1ff; cursor:pointer; transition:all .2s; margin:8px auto 4px; display:inline-flex; align-items:center; gap:5px; }
.gb-globe-btn:hover { background:rgba(0,225,255,.12); border-color:#00e1ff; transform:translateY(-1px); }
...
`;
  ...
  const KB = [
    ...
    {keys:['fluid','navier stokes','shader','webgl','glsl','background','particle','gpu','framebuffer','ping pong'],cat:'skill',answer:`The site background runs a <strong>Navier-Stokes fluid simulation</strong> on the GPU using WebGL fragment shaders and ping-pong framebuffers. It solves the incompressible Euler equations (advection, pressure-projection via 20 Jacobi iterations, vorticity confinement) at 60fps. The fluid reacts to your cursor, site audio via an AnalyserNode, and live Binance BTC trade shockwaves. Type <span class="highlight">fluid-stats</span> to see simulation metrics.`},
  ...
  function appendFollowUps(el, result) {
    ...
    if (fups.length) {
      ...
      fups.forEach(t => { h += `<span class="term-chat-topic" onclick="document.getElementById('termInput').value='ask ${t}';document.getElementById('termInput').dispatchEvent(new KeyboardEvent('keydown',{key:'Enter'}))">${t}</span>`; });
      h += '</div></div>';
    }
    h += '<div class="term-chat-topics" style="margin-top:6px">';
    TOPICS.forEach(t => { h += `<span class="term-chat-topic" onclick="document.getElementById('termInput').value='ask ${t}';document.getElementById('termInput').dispatchEvent(new KeyboardEvent('keydown',{key:'Enter'}))">${t}</span>`; });
    h += '</div>';
    ...
    function connectWS() {
      ...
      ws.onmessage = (evt) => {
        try {
          ...
          if (window._fluidSplat) {
            const val = parseFloat(trade.q) * parseFloat(trade.p);
            if (val > 10000) window._fluidSplat(
              Math.random(), Math.random(),
              (trade.m ? -1 : 1) * 0.3, 0,
              trade.m ? 0.9 : 0.1, trade.m ? 0.3 : 0.8, 0.2,
              val > 100000 ? 0.005 : 0.002
            );
          }
        } catch(e) {}
      };
  ...
  function launchGlobe() {
    open3D('🌐 Global Mentorship Reach', buildGlobeScene, 'three');
  }
  ...
  function buildGlobeScene(container) {
    ...
    const globeMat = new THREE.MeshBasicMaterial({ color: 0x0a1628, transparent: true, opacity: 0.92 });
    ...
    const arcData = [];
    const ARC_PTS = 60;
    const ARC_COLORS = [0xa855f7, 0x6366f1, 0x22c55e, 0x00e1ff];
    const disposables = [];

    function addLocation(lat, lng, colorIdx, delay) {
      if (!alive) return;
      const pos = latLngToVec3(lat, lng, 1.008);
      const color = ARC_COLORS[colorIdx % ARC_COLORS.length];
    ...
    MENTEE_LOCATIONS.forEach((loc, i) => {
      addLocation(loc.lat, loc.lng, i, i * 200 + Math.random() * 300);
    });

    if (window._sb) {
      window._sb.from('guestbook_entries').select('*')
        .not('lat', 'is', null)
        .order('created_at', { ascending: false })
        .limit(30)
        .then(({ data }) => {
          if (data) data.forEach((e, i) => {
            if (e.lat && e.lng) addLocation(e.lat, e.lng, i + MENTEE_LOCATIONS.length, MENTEE_LOCATIONS.length * 200 + i * 300);
          });
        }).catch(() => {});
    }
    ...
    hud.innerHTML = `<div></div><div>
      <div class="globe-stat-row">
        <div class="globe-stat"><div class="globe-stat-val">20+</div><div class="globe-stat-label">Countries</div></div>
        <div class="globe-stat"><div class="globe-stat-val">2,400+</div><div class="globe-stat-label">Minutes</div></div>
        <div class="globe-stat"><div class="globe-stat-val">50+</div><div class="globe-stat-label">Mentees</div></div>
      </div>
      <div class="globe-legend">
        <span><span class="globe-legend-dot" style="background:#00e1ff"></span>Cairo (Home)</span>
        <span><span class="globe-legend-dot" style="background:#a855f7"></span>Mentees</span>
