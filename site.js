...
// DB keepalive — ping every 3 days to keep Supabase free-tier alive
if (_sb) setInterval(async () => {
  try { await _sb.from('site_visits').select('id', { count: 'exact', head: true }); }
  catch (_) { /* silent */ }
}, 259200000);
...
(function(){
    ...
    function resetTimer(){clearTimeout(collapseTimer);if(expanded)collapseTimer=setTimeout(collapse,3000);}
    tog.addEventListener('click',function(e){
        ...
        if(expanded)collapseTimer=setTimeout(collapse,3000);
    });
    tb.addEventListener('mouseenter',function(){clearTimeout(collapseTimer);});
    tb.addEventListener('mouseleave',function(){if(expanded)collapseTimer=setTimeout(collapse,3000);});
...
document.getElementById('pfw').addEventListener('dblclick',e=>{e.preventDefault();showEgg();});
...
const VDna=(function(){
    ...
    save();
    return{get:()=>p,save,addXp(n){const mult=typeof window._mpXpMultiplier==='function'?window._mpXpMultiplier():1;p.xp+=Math.round(n*mult);const lvls=[0,50,150,300,500,800,1200];const nl=lvls.findIndex(t=>p.xp<t);p.level=nl===-1?lvls.length:nl;save();updateXpUI();},addClick(id){if(!p.clickedLinks.includes(id)){p.clickedLinks.push(id);save();}p.sessionClicks++;if(window._lake)window._lake.logEvent('click',id);},addSection(id){if(!p.sectionsViewed.includes(id)){p.sectionsViewed.push(id);save();}if(window._lake)window._lake.logEvent('section',id);},setScroll(pct){let changed=false;if(pct>p.scrollMax){p.scrollMax=pct;changed=true;}p.lastScrollY=window.scrollY;localStorage.setItem(k,JSON.stringify(p));if(changed)lakeSyncField('scrollMax',pct);lakeSyncField('lastScrollY',p.lastScrollY);}};
})();
window.VDna = VDna;
...
(function initAmbientLight() {
    ...
    function applyFromLux(lux) {
        ...
        debounce = setTimeout(() => {
            const isCp = document.body.classList.contains('cyberpunk-mode');
            const isLt = document.body.classList.contains('light-mode');
            const current = isCp ? 'cyberpunk' : isLt ? 'light' : 'dark';
            if (current === target) return;

            if (isCp && target !== 'cyberpunk' && window._toggleCyberpunk) window._toggleCyberpunk(true);

            if (target === 'cyberpunk') {
                applyTheme('dark'); localStorage.setItem('theme', 'dark'); _lakePref('theme','dark');
            ...
            const labels = { cyberpunk: '🌆 Cyberpunk', dark: '🌙 Dark', light: '☀️ Light' };
...
function showLevelUpToast(item){
    ...
    setTimeout(()=>toast.classList.add('show'),50);
    setTimeout(()=>{toast.classList.remove('show');setTimeout(()=>toast.remove(),500);},4200);
    if(navigator.vibrate&&window._userHasInteracted)try{navigator.vibrate([100,80,100,80,100,80,300]);}catch(e){}
}
...
function updateXpUI(){
    ...
    if(xpLvl){
        ...
        let bgResult;
        if(color.startsWith('#')){const r=parseInt(color.slice(1,3),16),g=parseInt(color.slice(3,5),16),b=parseInt(color.slice(5,7),16);bgResult=`rgba(${r},${g},${b},0.1)`;}
        else{bgResult=color.replace(')',',0.1)').replace('var(','rgba(').replace('--sub','107,122,144').replace('--accent','0,225,255');}
        xpLvl.style.background=bgResult;
    }
}
...
function openTrophy(){
    ...
    const trBtn=document.getElementById('trophyBtn'); if(trBtn) trBtn.classList.add('active');
    autoDismiss('trophyOverlay',closeTrophy);
}
window.closeTrophy=function(){const el=document.getElementById('trophyOverlay');if(el)el.classList.remove('show');const trBtn=document.getElementById('trophyBtn'); if(trBtn) trBtn.classList.remove('active');cancelAutoDismiss('trophyOverlay');};
// Share progress
...
const TermCmds = {
    ...
    _meta: {
        ...
        cyberpunk:{ cat: 'TOOLS', desc: 'Toggle cyberpunk theme' },
    ...
    _aliases: {
        ...
        sound: 'audio', neon: 'cyberpunk', nightcity: 'cyberpunk',
    ...
    cd: function(args, tab) {
        ...
        return `<span class="term-red">cd: ${t}: No such directory</span>`;
    },

    cat: function(args, tab) {
        const t = (args || '').trim();
        const dir = this.getDir(tab.path);
        if (dir[t] && typeof dir[t] === 'string') return `<span class="term-white">${dir[t]}</span>`;
        return `<span class="term-red">cat: ${t}: Cannot open file</span>`;
    },
...
<span class="term-cyan"> /_/        \\_\\ </span>  <span class="term-cyan">AI</span>: ${window._llmReady ? '<span class="term-green">Sentient</span>' : '<span class="term-gray">Keyword</span>'}`,
    ...
    weather: () => {
        ...
        return `<span class="term-cyan">📍 Cairo</span> · Temp: ${w.data ? Math.round(w.data.temp) : '--'}°C`;
    },
    theme: () => {
...
const _termTabMgr = {
    ...
    renameTab(name) {
        ...
        return '<span class="term-green">Renamed to ' + n + '</span>';
    },
...
function loadArcade(){
  if(window._prefetch)window._prefetch.recordHit('arcade.js');
...
`;
  ...
  function initZenMode() {
    ...
    audioBtn.insertAdjacentElement('afterend', spatialBtn);
    ...
    const zenBtn = document.getElementById('zenBtn') || (() => {
      ...
      spatialBtn.insertAdjacentElement('afterend', b);
      return b;
    })();
    ...
    function applyZen(active, animate) {
      ...
      if (active) {
        ...
        localStorage.setItem('cyberpunkMode', '0'); _lakePref('cyberpunkMode','0');
      }
...
function _noop(){}
window._openArcade = function(){ window._loadArcade().then(()=>{ if(window._openArcade) window._openArcade(); }).catch(_noop); };
window._closeArcade = function(){ if(window._arcadeLoaded && window._arcadeLoaded() && window._closeArcade) window._closeArcade(); };
window._closeMG = function(){ if(window._arcadeLoaded && window._arcadeLoaded() && window._closeMG) window._closeMG(); };
window._launchGame = function(id){ window._loadArcade().then(()=>{ if(window._launchGame) window._launchGame(id); }).catch(_noop); };
...
window.TermCmds.play = function(arg){ window._loadArcade().then(()=>{ if(window.TermCmds.play){var o=window.TermCmds.play(arg);_termAppend(o);} }).catch(_noop); return '<span class="term-green">Loading Arcade...</span>'; };
...
function loadAI3D(){
  if(window._prefetch)window._prefetch.recordHit('ai3d.js');
...
function loadSpatial(){
  if(window._prefetch)window._prefetch.recordHit('spatial-controller.js');
  ...
  spatialLoadPromise=new Promise((resolve,reject)=>{
    const s=document.createElement('script');s.src='spatial-controller.js';
...
function loadMesh(){
  ...
  meshLoadPromise=new Promise((resolve,reject)=>{
    const s=document.createElement('script');s.src='mesh.js';
...
function loadPrefetch(){
  ...
  prefetchLoadPromise=new Promise((resolve,reject)=>{
    const s=document.createElement('script');s.src='prefetch.js';
...
function loadPressure(){
  ...
  pressureLoadPromise=new Promise((resolve,reject)=>{
    const s=document.createElement('script');s.src='compute-pressure.js';
...
function loadBiometric(){
  ...
  biometricLoadPromise=new Promise((resolve,reject)=>{
    const s=document.createElement('script');s.src='biometric-risk.js';
...
function loadSonification(){
  ...
  sonoLoadPromise=new Promise((resolve,reject)=>{
    const s=document.createElement('script');s.src='sonification.js';
...
function loadCrossWindow(){
  ...
  cwLoadPromise=new Promise((resolve,reject)=>{
    const s=document.createElement('script');s.src='crosswindow.js';
...
function loadLake(){
  ...
  lakeLoadPromise=new Promise((resolve,reject)=>{
    const s=document.createElement('script');s.src='datalake.js';
  ...
  function initCyberpunk() {
    ...
    window._toggleCyberpunk = function(animate) {
      ...
      if (!document.body.classList.contains('cyberpunk-mode')) {
          ...
          localStorage.setItem('zenMode', '0'); _lakePref('zenMode','0');
      ...
      localStorage.setItem('cyberpunkMode', active ? '1' : '0'); _lakePref('cyberpunkMode',active?'1':'0');
      if (animate && !RM) {
        flash.classList.add('active');
...
`;
  ...
  function showTrophyToast(id) {
    if (!window._isSiteReady || !window._isSiteReady()) { setTimeout(() => showTrophyToast(id), 600); return; }
    const trophy = TROPHIES.find(t => t.id === id);
    if (!trophy) return;
  ...
  function initInteractiveTimeline() {
    ...
    ['all', ...Object.keys(TAGS)].forEach(tag => {
      ...
      btn.addEventListener('click', () => { if (navigator.vibrate) navigator.vibrate(8); filterTimeline(tag); });
  ...
  function initGuestbook() {
    ...
    overlay.innerHTML = `
      <div class="gb-panel">
        ...
        <div style="text-align:center"><span class="gb-globe-btn" onclick="window._dbg=window._dbg||[];window._dbg.push({h:'J',loc:'gb-globe-btn',msg:'globe-click',ai3dLoaded:typeof ai3dLoaded!=='undefined'?ai3dLoaded:'N/A',hasLaunchGlobe:typeof window._launchGlobe,hasTHREE:typeof THREE,t:Date.now()});if(window._launchGlobe){window._closeGuestbook();setTimeout(window._launchGlobe,300);}else{window._dbg.push({h:'J',loc:'gb-globe-btn',msg:'loading-ai3d',t:Date.now()});if(typeof loadAI3D==='function'){loadAI3D().then(function(){window._dbg.push({h:'J',loc:'gb-globe-btn',msg:'ai3d-loaded-ok',hasLaunchGlobe:typeof window._launchGlobe,t:Date.now()});if(window._launchGlobe){window._closeGuestbook();setTimeout(window._launchGlobe,300);}}).catch(function(e){window._dbg.push({h:'J',loc:'gb-globe-btn',msg:'ai3d-load-fail',err:String(e),t:Date.now()});});}else{window._dbg.push({h:'J',loc:'gb-globe-btn',msg:'no-loadAI3D-fn',t:Date.now()});}}">🌐 View Globe</span></div>
  ...
  function openGuestbook() {
    ...
    el.classList.add('show');
    // #region agent log
    setTimeout(()=>{try{window._dbg=window._dbg||[];const isLight=document.body.classList.contains('light-mode');const panel=el.querySelector('.gb-panel');const msg=el.querySelector('.gb-entry-msg');const time=el.querySelector('.gb-entry-time');const nm=el.querySelector('.gb-entry-name');const sub=el.querySelector('.gb-subtitle');const empty=el.querySelector('.gb-empty');const close=el.querySelector('.gb-close');const inp=el.querySelector('.gb-name-input');const cs=e=>e?getComputedStyle(e):null;window._dbg.push({h:'F-I',loc:'openGuestbook',msg:'gb-styles',theme:isLight?'light':'dark',panelBg:cs(panel)?.backgroundColor,msgColor:cs(msg)?.color,msgFs:cs(msg)?.fontSize,timeColor:cs(time)?.color,timeFs:cs(time)?.fontSize,nameColor:cs(nm)?.color,subColor:cs(sub)?.color,subFs:cs(sub)?.fontSize,emptyColor:cs(empty)?.color,closeColor:cs(close)?.color,closeOp:cs(close)?.opacity,inpColor:cs(inp)?.color,inpBorder:cs(inp)?.borderColor,t:Date.now()});}catch(e){}},500);
    // #endregion
  ...
  function closeGuestbook() { if (window._sono) window._sono.guestbookClose(); document.getElementById('guestbookOverlay')?.classList.remove('show'); const gbBtn=document.getElementById('guestbookBtn'); if(gbBtn) gbBtn.classList.remove('active'); cancelAutoDismiss('guestbookOverlay'); }
  ...
  function initVoiceNav() {
    ...
    const spatialBtnRef = document.getElementById('spatialBtn');
    if (spatialBtnRef) spatialBtnRef.insertAdjacentElement('beforebegin', btn);
    else { const zenBtnRef = document.getElementById('zenBtn'); if (zenBtnRef) zenBtnRef.insertAdjacentElement('beforebegin', btn); else document.getElementById('topBtns')?.appendChild(btn); }
...
<span class="term-gray">Features:</span> zen mode, cyberpunk, terminal, guestbook, trophy case, search, read book, spatial, mesh
...
<span class="term-cyan">Themes & Audio:</span>
  ...
  <span class="term-white">read book</span>         TTS audiobook preview with visualizer
  <span class="term-white">ambient</span>           Toggle ambient light auto-theme

<span class="term-cyan">Spatial Navigation:</span>
  <span class="term-white">spatial</span>           Toggle webcam spatial nav (eye scroll + hand gestures)
  <span class="term-white">spatial-help</span>      Show gesture guide
...
<span class="term-cyan">P2P Data Mesh:</span>
...
<span class="term-cyan">Local AI (WebGPU LLM):</span>
  ...
  <span class="term-white">llm-load</span>          Manually download the AI model
  <span class="term-white">llm-off</span>           Disable AI, use keyword matching only
  <span class="term-white">llm-on</span>            Re-enable local AI mode

<span class="term-cyan">Predictive Prefetching:</span>
  <span class="term-white">prefetch</span>          Toggle prefetch debug HUD
  <span class="term-white">prefetch-stats</span>    Prediction accuracy, model type, hit rate

<span class="term-cyan">Data Lake (OPFS + SQLite):</span>
  <span class="term-white">lake</span>              Data Lake status (OPFS/fallback)
  <span class="term-white">lake-stats</span>        Table counts, DB size, stores
  <span class="term-white">lake-export</span>       Download portable .db file
  <span class="term-white">lake-sql</span>           Run raw SQL queries on the lake

<span class="term-cyan">Behavioral Biometrics:</span>
  <span class="term-white">risk-profile</span>       Real-time behavioral analysis scatter plot + confidence score
...
<span class="term-gray">Events:</span>     ${s.eventCount} logged`);
      }).catch(e => { _termAppend('<span class="term-red">Error: ' + e.message + '</span>'); });
    ...
    T['lake-export'] = () => {
      ...
      }).catch(e => { _termAppend('<span class="term-red">Export error: ' + e.message + '</span>'); });
    ...
    T['lake-sql'] = (args) => {
      ...
      window._lake.exec(sql).then(r => {
        ...
        let out = '<span class="term-cyan">' + cols.join(' | ') + '</span>\n';
        out += '<span class="term-gray">' + cols.map(c => '─'.repeat(Math.max(c.length, 6))).join('─┼─') + '</span>\n';
        r.rows.slice(0, 50).forEach(row => {
          out += cols.map(c => { let v = row[c]; if (v === null || v === undefined) v = 'NULL'; v = String(v); return v.length > 40 ? v.slice(0, 37) + '...' : v; }).join(' | ') + '\n';
        });
        if (r.rows.length > 50) out += '<span class="term-gray">...and ' + (r.rows.length - 50) + ' more rows</span>';
        _termAppend(out);
      }).catch(e => { _termAppend('<span class="term-red">SQL error: ' + e.message + '</span>'); });
...
(function() {
    ...
    const UniToast = {
        ...
        render({ title, meta, icon, type }) {
            ...
            if (navigator.vibrate && window._userHasInteracted) try { navigator.vibrate(15); } catch(e) {}
