...
(function FluidSim() {
  ...
  document.body.insertBefore(canvas, bgC);
  ...
  function connectTrades() {
    ...
    try {
      ...
      tradeWS.onclose = () => {
        tradeWS = null;
        if (running) setTimeout(connectTrades, Math.min(tradeReconnect *= 2, 16000));
      };
    } catch (_) {}
  }
  ...
  function destroy() {
    ...
    canvas.remove();
    if (bgC) bgC.style.display = '';
    const oldScript = document.querySelector('script[src="fluid.js"]');
