// chatbot-llm.js — "Ask Amr" terminal chatbot, KB, LLM engine, streaming
...
.llm-progress { position: sticky; bottom: 0; z-index: 5; height: 22px; background: rgba(0,225,255,.04); border: 1px solid rgba(0,225,255,.1); border-radius: 6px; margin-top: 8px; overflow: hidden; font-family: 'JetBrains Mono', monospace; backdrop-filter: blur(12px); }
...
.light-mode .llm-progress { background: rgba(0,102,255,.06); border-color: rgba(0,102,255,.15); }
.light-mode .llm-progress-bar { background: linear-gradient(90deg, rgba(0,102,255,.18), rgba(99,102,241,.22)); }
.light-mode .llm-progress-label { color: rgba(30,40,60,.7); }
.light-mode .term-chat-q { background: rgba(0,102,255,.05); border-left-color: rgba(0,102,255,.35); color: #1f2937; }
.light-mode .term-chat-a { background: rgba(0,0,0,.02); border-left-color: rgba(0,0,0,.08); color: #374151; }
.light-mode .term-chat-a strong { color: #111827; }
.light-mode .term-chat-a .highlight { color: #0066ff; text-shadow: none; }
.light-mode .term-chat-typing span { background: #0066ff; box-shadow: 0 0 4px rgba(0,102,255,.3); }
.light-mode .term-chat-topic { border-color: rgba(0,102,255,.15); background: rgba(0,102,255,.04); color: #0066ff; text-shadow: none; }
.light-mode .term-chat-topic:hover { background: rgba(0,102,255,.1); border-color: rgba(0,102,255,.3); }
.light-mode .term-chat-cursor { background: #0066ff; box-shadow: 0 0 6px rgba(0,102,255,.4); }
.light-mode .term-chat-confidence.good { background: rgba(0,102,255,.06); color: #0066ff; border-color: rgba(0,102,255,.12); text-shadow: none; }
.light-mode .term-chat-followups { border-top-color: rgba(0,0,0,.06); }
.light-mode .term-chat-followup-label { color: #6b7280; }
.light-mode .term-chat-badge.loading { background: rgba(0,102,255,.06); color: #0066ff; border-color: rgba(0,102,255,.12); }
`;
  ...
  if (window._KB_DOCS && Array.isArray(window._KB_DOCS)) {
    KB.push(...window._KB_DOCS);
  }
  ...
  let _llmHistory = [];
  let _llmReadyResolve = null;
  let _llmReadyPromise = null;

  function buildSystemPrompt() {
    return {
      role: 'system',
      content: 'You are Amr Elharony\'s AI assistant on his portfolio website. Answer ONLY from the context the user provides. Be concise (2-3 sentences). Speak as Amr in first person. NEVER invent links, prices, or details not in the context.'
    };
  }

  function buildUserMessage(question) {
    let ctx = '';
    const kbResult = matchQuery(question);
    if (kbResult.matches.length) {
      kbResult.matches.forEach(m => {
        const plain = m.entry.answer.replace(/<[^>]+>/g, '').replace(/\s+/g, ' ').trim();
        ctx += plain + '\n\n';
      });
    }
    if (ctx) {
  ...
  async function initLLM() {
    if (_llmReady || _llmLoading || _llmDisabled || A.isMobile) return;
    if (!navigator.gpu) return;
    const _devMem = navigator.deviceMemory || 8;
    const _devCores = navigator.hardwareConcurrency || 4;
    if (_devMem <= 4 || (_devMem <= 6 && _devCores <= 2)) {
      console.info('[LLM] Low-memory device (%.1fGB, %d cores) — staying in KB mode', _devMem, _devCores);
      _llmDisabled = true;
      _llmLoading = false;
      if (window.UniToast) window.UniToast('Limited GPU memory — using Knowledge Base mode for stability');
      return;
    }
    if (window.lockdown || window.harden || typeof Object.isFrozen === 'function' && Object.isFrozen(Array.prototype)) {
      console.warn('SES lockdown detected — a browser extension may block AI model loading. Try incognito mode.');
      if (window.UniToast) window.UniToast('\u26a0\ufe0f Browser extension may block AI — try incognito mode');
    }
    _llmLoading = true;
    _llmReadyPromise = new Promise(r => { _llmReadyResolve = r; });
    showLLMProgressBar();
    _llmProgress = { text: 'Checking GPU...', progress: 0 };
    updateLLMProgressUI();
    try {
      const adapter = await navigator.gpu.requestAdapter();
      if (!adapter) { _llmLoading = false; if (_llmReadyResolve) _llmReadyResolve(false); updateLLMProgressUI(); return; }
    } catch (e) { if(window.AmrOS)window.AmrOS.logError('ChatbotLLM','WebGPU requestAdapter: '+((e&&e.message)||e)); _llmLoading = false; if (_llmReadyResolve) _llmReadyResolve(false); updateLLMProgressUI(); return; }
    _llmProgress = { text: 'Loading AI library...', progress: 0.02 };
    updateLLMProgressUI();
    if (window.UniToast) window.UniToast('Downloading local AI model... (cached after first load)');

    try {
      const { CreateMLCEngine } = await import('https://esm.run/@mlc-ai/web-llm');
      _llmProgress = { text: 'Downloading model...', progress: 0.05 };
      updateLLMProgressUI();
      _llmEngine = await CreateMLCEngine(LLM_MODEL, {
      ...
      _llmReady = true;
      _llmLoading = false;
      if (_llmReadyResolve) _llmReadyResolve(true);
      updateLLMProgressUI();
      if (typeof _termUpdateStatus === 'function') _termUpdateStatus();
      ...
      _llmLoading = false;
      _llmReady = false;
      if (_llmReadyResolve) _llmReadyResolve(false);
      updateLLMProgressUI();
      if (location.protocol === 'file:') {
        if (window.UniToast) window.UniToast('LLM requires an HTTP server — use Live Server or npx serve');
      }
    }
  }
  ...
  // ═══════════════════════════════════════════════════
  function _detectRepetition(text) {
    const tail = text.slice(-80);
    for (let len = 2; len <= 20; len++) {
      const pat = tail.slice(-len);
      let count = 0;
      for (let i = tail.length - len; i >= 0; i -= len) {
        if (tail.slice(i, i + len) === pat) count++;
        else break;
      }
  ...
  function _trimRepetition(text) {
    ...
    for (let len = 2; len <= 20; len++) {
      ...
      if (count >= 4) {
        const cutFrom = text.length - (count * len);
        let trimmed = text.slice(0, cutFrom).trimEnd();
  ...
  async function streamLLMAnswer(containerId, question, sid) {
    ...
    const userMsg = buildUserMessage(question);
    const recentHistory = _llmHistory.length > 10 ? _llmHistory.slice(-10) : _llmHistory;
    const pendingHistory = [...recentHistory, { role: 'user', content: userMsg }];
    const messages = [systemPrompt, ...pendingHistory];
    let fullText = '';
    let tokenCount = 0;
    ...
    try {
      ...
      for await (const chunk of completion) {
        ...
        if (_detectRepetition(fullText)) { fullText = _trimRepetition(fullText); break; }
      ...
      _llmHistory = [...recentHistory, { role: 'user', content: question }, { role: 'assistant', content: fullText }];
      ...
      if (result.matches.length) {
        el.insertAdjacentHTML('beforeend', result.matches.map(m => m.entry.answer).join('<br><br>'));
      } else {
        el.insertAdjacentHTML('beforeend', 'Sorry, the AI encountered an error. Here are some topics you can ask about:');
      }
      appendFollowUps(el, result);
    }
  }
  ...
  function formatWelcomeLLM() {
    let mode;
    if (_llmReady) mode = '\ud83e\udde0 <strong>Sentient Mode</strong> (local AI active)';
    else if (_llmLoading) mode = '\u23f3 AI model loading...';
    else mode = '\ud83d\udcda Knowledge Base mode';
    let h = `${mode}\nAsk me anything about Amr's <strong>career, certifications, book, articles, mentoring, conferences</strong>, and more.\n\nTry: <span class="highlight">"Tell me about your book"</span> or <span class="highlight">"What certifications do you have?"</span>`;
  ...
  function initAskAmr() {
    ...
    window.TermCmds.ask = function(args) {
      ...
      let html = `<div class="term-chat-msg term-chat-q">\u2753 ${A.escHtml(q)}</div>`;
      ...
      } else if (_llmLoading && !forceLocal && !_llmDisabled && _llmReadyPromise) {
        const el = document.getElementById(streamId);
        if (el) el.innerHTML = '<span class="term-cyan">\u23f3 AI model is loading — your answer will stream when ready...</span>';
        _llmReadyPromise.then(ok => {
          if (myId !== _streamId) return;
          if (ok && _llmReady) {
            streamLLMAnswer(streamId, q, myId);
          } else {
            const result = matchQuery(q);
            streamAnswer(streamId, result, myId);
          }
        });
      } else {
        const result = matchQuery(q);
    ...
    window.TermCmds.llm = function() {
      if (_llmReady) {
        ...
        let out = '<span class="term-green">\ud83e\udde0 Sentient Mode Active</span>\n';
      ...
      if (_llmLoading) return '<span class="term-cyan">\u23f3 Model downloading...</span>\n<span class="term-gray">' + _llmProgress.text + '</span>';
      if (_llmDisabled) return '<span class="term-gray">Local LLM disabled — type <span class="term-white">llm-on</span> to re-enable</span>';
      if (A.isMobile) return '<span class="term-gray">Local LLM not available on mobile (insufficient GPU memory)</span>';
      if (!navigator.gpu) return '<span class="term-gray">WebGPU not supported — local LLM unavailable</span>';
      return '<span class="term-gray">Local LLM not loaded — type <span class="term-white">llm-load</span> to download</span>';
    };
    ...
    window.TermCmds['llm-load'] = function() {
      ...
      if (_llmLoading) {
        showLLMProgressBar();
        return '<span class="term-cyan">\u23f3 Already downloading...</span>';
      }
      _llmDisabled = false;
      initLLM();
