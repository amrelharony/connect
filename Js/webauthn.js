...
(function PasskeyManager() {
  ...
  function setSession(credId) {
    ...
    try { sessionStorage.setItem(LS_SESSION, JSON.stringify({ credId, ts: Date.now() })); } catch (_) {}
    if (window._lake && window._lake.isReady) {
  ...
  function restoreSession() {
    try {
      ...
      if (!s.credId || (Date.now() - s.ts) >= 86400000) return;
      const creds = loadCredentials();
  ...
  async function syncToSupabase(credData) {
    ...
    try {
      ...
      await window._sb.from('passkey_profiles').upsert({
