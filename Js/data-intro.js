...
(function DataIntroEngine() {
  ...
  var PALETTES_LIGHT = {
    clear: { primary: '#0055ee', secondary: '#4f46e5', tertiary: '#7c3aed', glow: 'rgba(0,85,238,0.35)', bg: '#f4f6fb', glass: 'rgba(0,20,60,0.06)', border: 'rgba(0,20,60,0.15)', speed: 1 },
    cloud: { primary: '#3e4c5e', secondary: '#2d3a4a', tertiary: '#5a6a7e', glow: 'rgba(62,76,94,0.30)', bg: '#eef1f8', glass: 'rgba(0,0,30,0.07)', border: 'rgba(0,0,30,0.18)', speed: 0.6 },
    rain:  { primary: '#1d5ed8', secondary: '#1e40af', tertiary: '#3b7cf0', glow: 'rgba(29,94,216,0.35)', bg: '#edf2ff', glass: 'rgba(0,30,80,0.07)', border: 'rgba(0,30,80,0.18)', speed: 0.8 },
    storm: { primary: '#6d28d9', secondary: '#c42020', tertiary: '#8b5cf6', glow: 'rgba(109,40,217,0.35)', bg: '#f5f0ff', glass: 'rgba(60,0,120,0.07)', border: 'rgba(60,0,120,0.18)', speed: 1.4 }
  };
  ...
  function buildFilterDefs(atmo, temp, wind, isLight) {
    ...
    return '<defs>' +
      '<filter id="diTempColor" color-interpolation-filters="sRGB"><feColorMatrix type="matrix" values="' + matrix + '"/></filter>' +
  ...
  function typewriterLabel(text, x, y, pal, beginTime, isLight) {
    var fill = isLight ? '#0a0f1a' : pal.primary;
    var svg = '<text x="' + x + '" y="' + y + '" text-anchor="middle" font-family="\'JetBrains Mono\',monospace" font-size="12" letter-spacing="8" font-weight="' + (isLight ? '600' : '500') + '" filter="url(#diGlowSoft)">';
    for (var i = 0; i < text.length; i++) {
      var ch = text[i] === ' ' ? '&#160;' : text[i];
      var d = (beginTime + i * 0.05).toFixed(2);
      svg += '<tspan fill="' + fill + '" fill-opacity="0">' + ch +
        '<animate attributeName="fill-opacity" from="0" to="0.8" dur="0.12s" begin="' + d + 's" fill="freeze"/>' +
        '<animate attributeName="fill-opacity" from="0.8" to="0" dur="0.4s" begin="3.2s" fill="freeze"/></tspan>';
    }
    svg += '</text>';
    return svg;
  }
  ...
  function buildFullSVG(phase, atmo, temp, wind) {
    ...
    var pal = getPalette(atmo, isLight);
    var bgColor = _livePal ? pal.bg : ((!isLight && PHASE_BG[phase]) ? PHASE_BG[phase] : pal.bg);
