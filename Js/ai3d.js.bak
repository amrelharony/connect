...
`;
  ...
  async function initLLM() {
    ...
    } catch (_) { _llmLoading = false; if (_llmReadyResolve) _llmReadyResolve(false); updateLLMProgressUI(); return; }
  ...
  function initAskAmr() {
    ...
    window.TermCmds.llm = function() {
      ...
      if (isMobile) return '<span class="term-gray">Local LLM not available on mobile (insufficient GPU memory)</span>';
