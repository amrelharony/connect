...
(function PresenceEngineV2() {
  ...
  if (!_sb) { console.warn('[Presence] Supabase client (_sb) not found — presence engine disabled.'); return; }
  ...
  const ADJECTIVES = ['Swift','Bold','Bright','Calm','Clever','Cosmic','Daring','Epic','Fierce','Grand','Happy','Keen','Lucky','Noble','Quick','Sharp','Silent','Steady','Vivid','Wise','Agile','Brave','Cool','Fresh','Slick','Zen','Prime','Pixel','Neon','Cyber'];
  const NOUNS = ['Falcon','Phoenix','Tiger','Panda','Eagle','Wolf','Otter','Fox','Hawk','Lion','Dolphin','Owl','Lynx','Bear','Raven','Shark','Cobra','Crane','Stag','Viper','Coder','Spark','Pilot','Scout','Knight','Nomad','Sage','Maven','Rider','Atlas'];
  function generateFriendlyName(seed) {
    let h = 0;
  ...
  class NetworkOptimizer {
    ...
    handleGossip(payload) {
      if (payload.type === 'gossip_digest') {
        ...
        for (const [pid, ver] of Object.entries(payload.digest || {})) {
          const localVer = this.gossipVersions.get(pid) || 0;
          if (ver > localVer) { this.gossipVersions.set(pid, ver); hasNewer = true; }
        }
        if (hasNewer && window._mesh) {
  ...
  class PredictiveCore {
    ...
    // Kalman filter math delegated to Js/lib/kalman-filter.js
    _getOrCreateFilter(peerId) {
      let kf = this.filters.get(peerId);
      if (!kf) {
        kf = window.KalmanFilter2D.create();
        kf.lastUpdate = 0;
    ...
    updateFilter(peerId, mx, my) {
      ...
      if (!kf.initialized) {
        window.KalmanFilter2D.update(kf, mx, my);
        kf.lastUpdate = now;
        return;
      }
      ...
      kf.lastUpdate = now;
      window.KalmanFilter2D.predict(kf, dt);
      window.KalmanFilter2D.update(kf, mx, my);
    }
    ...
    predict(peerId, dt) {
      ...
      const pos = window.KalmanFilter2D.extrapolate(kf, dt);
      return { x: clamp01(pos.x), y: clamp01(pos.y), vx: pos.vx, vy: pos.vy };
    }
    ...
    computeEngagement(peerId, peerData) {
      ...
      const prevActive = ed.lastActive;
      ed.lastActive = now;
      ...
      const recency = Math.exp(-0.001 * (now - prevActive) / 1000);
  ...
  // Boids flocking math delegated to Js/lib/boids-swarm.js
  class SwarmIntelligence {
    ...
    updateBoids(renderedPeers) {
      const BS = window.BoidsSwarm;
      BS.updateBoids(this.boids, renderedPeers, {
        maxSpeed: this.MAX_SPEED, maxForce: this.MAX_FORCE,
        perception: this.PERCEPTION, sepDist: this.SEP_DIST,
        ghostsPerPeer: this.GHOSTS_PER_PEER
      });
    }
    ...
    detectFormations(renderedPeers) {
      ...
      this.formations = window.BoidsSwarm.detectFormations(positions, this.engine.physics.wells);

      // Side effects: award XP and record interactions for new formations
    ...
    getSwarmMetrics() {
      return window.BoidsSwarm.getSwarmMetrics(this.boids);
    }
  }
  ...
  class InteractionManager {
    ...
    handleCtxAction(action, targetId) {
      switch (action) {
        ...
        case 'call': this.startVoIPCall(targetId); break;
        case 'hangup': this.hangupVoIPCall(targetId); break;
      }
    }

    startVoIPCall(targetId) {
      const doCall = () => {
        if (!window._voip) { if (window.UniToast) window.UniToast('⚠️ VoIP module not available — mesh connection required'); return; }
        window._voip.call(targetId);
      };
      if (window._voip) { doCall(); return; }
      if (typeof window._loadVoIP === 'function') {
        window._loadVoIP().then(doCall).catch(() => { if (window.UniToast) window.UniToast('❌ Failed to load VoIP module'); });
      } else {
        if (window.UniToast) window.UniToast('⚠️ VoIP not available');
      }
    }

    hangupVoIPCall(targetId) {
      if (window._voip) window._voip.hangup(targetId);
    }
    ...
    handleBroadcastEvent(payload) {
      ...
      if (payload._mid) { if (this.engine._meshSeen.size > 5000) this.engine._meshSeen.clear(); this.engine._meshSeen.add(payload._mid); setTimeout(() => this.engine._meshSeen.delete(payload._mid), 15000); }
    ...
    sendHighFive(targetId) {
      ...
      if (window.UniToast) { const msg = '✋ High-five sent! Waiting for them to high-five back...'; typeof window.UniToast === 'function' ? window.UniToast(msg) : window.UniToast.add && window.UniToast.add(msg); }
    }
      ...
      for (const [pid, pArr] of Object.entries(peers)) {
        ...
        if (p.section && p.section === mySection && mySection !== 'hero') {
          if (!this.sameSecTimers[pid]) this.sameSecTimers[pid] = Date.now();
            ...
            if (window.UniToast) {
              const voipAvail = window._voip && !window._voip.isCalling(pid);
              const voipHint = voipAvail ? ' · 🎙️ Voice chat available' : '';
              const msg = `💬 ${escText(nick)} is also viewing ${escText(mySection)}${extra}${voipHint} — click their avatar`;
              typeof window.UniToast === 'function' ? window.UniToast(msg) : window.UniToast.add && window.UniToast.add(msg);
            }
    ...
    receiveWhisper(fromNick, text) {
      ...
      if (window.UniToast) { const msg = `💬 ${escText(fromNick)}: ${escText(text)}`; typeof window.UniToast === 'function' ? window.UniToast(msg) : window.UniToast.add && window.UniToast.add(msg); }
    }
    ...
    checkVSMode() {
      ...
      if (rival) {
        ...
        this.vsPanel.innerHTML = `<div class="mp-vs-label">VS</div><div class="mp-vs-nick">${escText(r.nickname || 'Anon')}</div><div class="mp-vs-score">${parseInt(r.gameState.score) || 0}</div>`;
    ...
    termBroadcast(text) {
      ...
      return `<span class="term-green">📡 Broadcast sent: "${escText(msg)}"</span>`;
    }
    ...
    receiveBroadcast(fromNick, text) {
      ...
      if (body) {
        body.insertAdjacentHTML('beforeend', `<div class="term-line"><span style="color:#a855f7">[broadcast]</span> <span style="color:#22c55e">${escText(fromNick)}</span>: ${escText(text)}</div>`);
    ...
    checkCoopUnlock() {
      ...
      if (hasBoth) {
        ...
        const body = document.getElementById('termBody');
        if (body) {
          body.insertAdjacentHTML('beforeend', `<div class="term-line"><span style="color:#22c55e;text-shadow:0 0 8px #22c55e">
╔══════════════════════════════════════╗
...
╚══════════════════════════════════════╝</span></div>`);
    ...
    startSpectate(targetId) {
      ...
      const pData = this.engine._getPeerData(targetId);
      this.engine.ui.specBanner.innerHTML = `👁 Spectating <strong>${escText((pData && pData.nickname) || 'Anon')}</strong> <span class="mp-spec-exit" id="mpSpecExit">✕ Exit</span>`;
    ...
    checkCriticalMass() {
      ...
      if (liveCount >= 5 && !this.engine.criticalMassActive) {
        ...
        if (window.UniToast) window.UniToast('⚡ Critical mass reached!');
    ...
    initCursorChat() {
      ...
      document.addEventListener('keydown', (e) => {
        ...
        if (e.key === '/' && !this.chatActive && (e.ctrlKey || e.metaKey)) { e.preventDefault(); this.openCursorChat(); return; }
        if (!this.chatActive) return;
  ...
  class UIRenderer {
    ...
    hidePresenceBar() {
      if (this.barWrap) this.barWrap.classList.remove('visible');
      document.querySelectorAll('.peer-cursor').forEach(el => el.remove());
    }

    init() {
      const vcEl = document.getElementById('visitorCount');
      if (!vcEl) { console.warn('[Presence] #visitorCount element not found — cannot mount presence bar.'); return; }
      ...
      for (let i = 0; i < this.MAX_RENDERED_CURSORS; i++) {
        ...
        el.innerHTML = '<span class="pe-cursor-avatar"></span><span class="pe-cursor-nick"></span>';
      ...
      for (let i = 0; i < this.MAX_RENDERED_CURSORS * 3; i++) {
        const el = document.createElement('div');
        el.style.cssText = 'position:fixed;top:0;left:0;z-index:9989;pointer-events:none;display:none;width:4px;height:4px;border-radius:50%;background:rgba(0,225,255,0.2);transition:transform 16ms linear;will-change:transform;';
        document.body.appendChild(el);
    ...
    renderBar() {
      ...
      this.barWrap.classList.add('visible');
      if (others > 0) {
        ...
        statusBox.innerHTML = `<span style="opacity:.5">You're here — avatars appear when others visit</span>`;
        statusBox.classList.add('visible');
      }

      let html = `<div class="mp-avatar-wrap"><div class="mp-avatar mp-avatar-me" title="You (${this.engine.myNickname})" data-id="${myId}">${emojiFromCode(this.engine.myAvatar)}</div><span class="mp-nick-label">${escText(this.engine.myNickname)}</span></div>`;
      for (const [pid, pArr] of peerList) {
        ...
        const peerNick = p.nickname || 'Anon';
        html += `<div class="mp-avatar-wrap"><div class="mp-avatar" title="${escText((p.nickname || 'Anonymous') + ': ' + status + predHint)}" data-id="${escText(pid)}" data-nick="${escText(peerNick)}" style="${extraStyle}">${emojiFromCode(p.avatar || '1F47E')}${trustBadge}</div><span class="mp-nick-label">${escText(peerNick)}</span></div>`;
      }
    ...
    showCtx(anchorEl, pid, pData) {
      ...
      const _pid = escText(pid);
      let items = `<div class="mp-ctx-title">${escText(nick)}</div>`;
      items += `<div class="mp-ctx-item" data-action="highfive" data-pid="${_pid}">✋ High-Five</div>`;
      if (sameSection) items += `<div class="mp-ctx-item" data-action="whisper" data-pid="${_pid}">💬 Whisper</div>`;
      items += `<div class="mp-ctx-item" data-action="tip" data-pid="${_pid}">🪙 Tip 5 XP</div>`;
      const voipActive = window._voip && window._voip.isCalling(pid);
      if (voipActive) {
        items += `<div class="mp-ctx-item" data-action="hangup" data-pid="${_pid}">📴 Hang Up</div>`;
      } else {
        items += `<div class="mp-ctx-item" data-action="call" data-pid="${_pid}">🎙️ Voice Call</div>`;
      }
      items += `<div class="mp-ctx-item" data-action="spectate" data-pid="${_pid}">👁 Spectate</div>`;
    ...
    renderPeerCursors(renderedPeers) {
      for (let i = 0; i < this.peerCursorPool.length; i++) {
        ...
        if (i < renderedPeers.length) {
          ...
          el.querySelector('.pe-cursor-avatar').textContent = emojiFromCode(p.avatar || '1F47E');
          el.querySelector('.pe-cursor-nick').textContent = p.nickname || 'Visitor';
        } else {
    ...
    renderPeerChat(fromId, fromNick, avatar, text, x, y) {
      ...
      if (!bubble) {
        ...
        bubble.innerHTML = `<span class="cc-avatar">${emojiFromCode(avatar || '1F47E')}</span><span class="cc-nick">${escText(fromNick)}</span><span class="cc-text"></span>`;
    ...
    removePeerChat(fromId) {
      ...
      if (bubble) {
        if (bubble._timer) clearTimeout(bubble._timer);
        delete this.peerChatBubbles[fromId];
  ...
  class PresenceEngine {
    constructor() {
      const syncUid = window.AuthManager ? window.AuthManager.currentUidSync() : null;
      this.myId = syncUid || (crypto.randomUUID ? crypto.randomUUID() : Math.random().toString(36).slice(2));
      this.myAvatar = AVATARS[Math.floor(Math.random() * AVATARS.length)];
      this.myNickname = localStorage.getItem('arcade_player_name') || generateFriendlyName(this.myId);
    ...
    boot() {
      ...
      // Upgrade myId to auth.uid() when a session is created later
      if (window.AuthManager) {
        window.AuthManager.onSession((session) => {
          if (session && session.user && session.user.id && session.user.id !== this.myId) {
      ...
      this.channel
        ...
        .subscribe(async (status, err) => {
          if (status === 'SUBSCRIBED') {
            ...
            this._channelErrors++;
            console.warn('[Presence] Channel error #' + this._channelErrors, err);
            if (this._channelErrors >= 2) {
              console.warn('[Presence] Too many errors — entering single-player mode');
              if (window.AmrOS) window.AmrOS.offlineMode = true;
              try { this.channel.unsubscribe(); } catch (_) {}
              try { this.evChannel.unsubscribe(); } catch (_) {}
              this.ui.hidePresenceBar();
              if (window.UniToast) window.UniToast('Multiplayer unavailable — enjoying single-player mode');
            }
          } else if (status === 'TIMED_OUT') {
            console.warn('[Presence] Channel timed out — retrying...');
            try { this.channel.subscribe(); } catch (_) {}
          }
        });
      ...
      this._scrollHandler = () => requestAnimationFrame(() => { if (!this._destroyed) this._trackMeta(false); });
      window.addEventListener('scroll', this._scrollHandler, { passive: true });
          ...
          if (window._mesh) {
            window._mesh.onMessage((peerId, msg) => {
              if (msg && typeof msg === 'object' && msg.type) {
                if (msg.type.startsWith('voip_')) {
                  this._handleVoIPFallback(peerId, msg);
                  return;
                }
                this.interactions.handleBroadcastEvent(msg);
              }
            });
            window._mesh.onPeerState((peerId, state) => {
              if (state === 'open') this._ensureVoIPLoaded();
            });
            window._mesh.syncPeers(Object.keys(this.channel.presenceState()));
            this._ensureVoIPLoaded();
          }
        }).catch(() => {});
      }
    ...
    _getScrollPct() { const max = document.documentElement.scrollHeight - window.innerHeight; return max > 0 ? Math.round((window.scrollY / max) * 100) : 0; }
    ...
    _trackMeta(force) {
      ...
      window._mpMyScrollPct = this.myScrollPct;
      ...
      if (delta || force) {
        this.channel.track(current).catch(() => {});
      }
    }
      ...
      try {
        ...
        for (const pid of oldPeerIds) {
          if (!state[pid]) {
            ...
            if (this.social._sameSecTimers) delete this.social._sameSecTimers[pid];
            if (this.social.influence) this.social.influence.delete(pid);
            if (this.interactions.pendingHighFives) delete this.interactions.pendingHighFives[pid];
            if (this.interactions.sameSecTimers) delete this.interactions.sameSecTimers[pid];
            this.network.gossipVersions.delete(pid);
          }
        }
        ...
        if (window._voip) {
          for (const [pid, pArr] of Object.entries(state)) {
            if (pid === this.myId) continue;
            const p = pArr[0];
            if (p && window._voip.isCalling(pid)) {
              window._voip.updatePeerAudio(pid, p);
            }
          }
        }
      } catch (e) {
        console.warn('[Presence] Sync handler error:', e);
      }
    }
    ...
    _ensureVoIPLoaded() {
      ...
      if (typeof window._loadVoIP === 'function') {
        window._loadVoIP().then(() => {
        }).catch(() => {});
      }
    }

    _handleVoIPFallback(peerId, msg) {
      
      if (window._voip) return;
      if (typeof window._loadVoIP === 'function') {
        const pendingPeerId = peerId;
        const pendingMsg = { ...msg };
        window._loadVoIP().then(() => {
          if (!window._voip) return;
          
          if (pendingMsg.type === 'voip_ring' && pendingMsg.from !== this.myId) {
            window._voip.call && window._voip.autoAnswer
              ? window._voip.autoAnswer(pendingPeerId)
              : null;
          }
        }).catch(() => {});
      }
  ...
  engine.boot();
  console.log('%c👥 Presence%c Engine v2 — id: '+engine.myId.slice(0,8)+' — '+emojiFromCode(engine.myAvatar),'background:#22c55e;color:#fff;padding:2px 6px;border-radius:3px;font-weight:bold;','color:#8a95a8;padding-left:6px;');
