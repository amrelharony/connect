...
╚══════════════════════════════════════╝</span></div>`;
  ...
  class UIRenderer {
    ...
    init() {
      ...
      for (let i = 0; i < this.MAX_RENDERED_CURSORS; i++) {
        ...
        el.style.cssText = 'position:fixed;top:0;left:0;z-index:9990;pointer-events:none;display:none;font-size:12px;transition:transform 16ms linear;will-change:transform;opacity:0.5;';
        el.innerHTML = '<span class="pe-cursor-emoji"></span><span class="pe-cursor-nick" style="font-size:8px;margin-left:4px;color:var(--sub);font-family:JetBrains Mono,monospace;white-space:nowrap;background:rgba(0,0,0,.55);padding:1px 5px;border-radius:4px;"></span>';
    ...
    renderBar() {
      ...
      for (const [pid, pArr] of peerList) {
        ...
        html += `<div class="mp-avatar" title="${escText((p.nickname || 'Anonymous') + ': ' + status + predHint)}" data-id="${escText(pid)}" data-nick="${escText(p.nickname || 'Anon')}" style="${extraStyle}">${emojiFromCode(p.avatar || '1F47E')}${trustBadge}</div>`;
      }
  ...
  class PresenceEngine {
    ...
    _ensureVoIPLoaded() {
      ...
      if (typeof window._loadVoIP === 'function') {
        console.log('[Presence] Loading VoIP module...');
        window._loadVoIP().then(() => {
          if (window._voip) console.log('[Presence] VoIP auto-loaded for incoming call support');
          else console.warn('[Presence] VoIP script loaded but window._voip not set — mesh may have been null');
        }).catch((e) => { console.warn('[Presence] VoIP load failed:', e); });
      }
    }

    _handleVoIPFallback(peerId, msg) {
      console.log('[Presence] VoIP fallback for', msg.type, 'from', peerId.slice(0, 8));
      if (window._voip) return;
      if (typeof window._loadVoIP === 'function') {
        ...
        window._loadVoIP().then(() => {
          if (!window._voip) return;
          console.log('[Presence] VoIP loaded, replaying', pendingMsg.type);
  ...
  engine.boot();
  console.log('[Presence] Engine v2 booted — id:', engine.myId.slice(0, 8), '— avatar:', emojiFromCode(engine.myAvatar));
