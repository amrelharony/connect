...
(function GamificationEngine() {
  ...
  function addXp(n) {
    ...
    window.dispatchEvent(new CustomEvent('AmrOS:XPChanged', { detail: { xp: p.xp, level: p.level } }));
  }
  ...
  function unlock(id) {
    ...
    window.dispatchEvent(new CustomEvent('AmrOS:AchievementUnlocked', { detail: { id: id } }));
    return true;
  }
  ...
  function createChallengeHUD() {
    ...
    updateChallengeHUD();
    var blogActive = document.getElementById('blogView');
    if(window._peripheralsShown || window._blogDirect || (blogActive && blogActive.classList.contains('active'))){challengeHUD.classList.add('show');var fab=document.getElementById('askFab');if(fab)fab.classList.add('show');}
  }
  ...
  function promptPlayerName(callback) {
    var existing = document.getElementById('gcNamePrompt');
    if (existing) existing.remove();
    var wrap = document.createElement('div');
    wrap.id = 'gcNamePrompt';
    wrap.style.cssText = 'display:flex;gap:6px;margin:8px 0;align-items:center';
    var inp = document.createElement('input');
    inp.type = 'text';
    inp.maxLength = 20;
    inp.placeholder = 'Your name';
    inp.style.cssText = 'flex:1;padding:6px 10px;border-radius:8px;border:1px solid var(--border);background:var(--card);color:var(--text);font-family:JetBrains Mono,monospace;font-size:12px;outline:none';
    var btn = document.createElement('button');
    btn.textContent = 'Join';
    btn.style.cssText = 'padding:6px 14px;border-radius:8px;border:none;background:var(--accent);color:#06080f;font-family:JetBrains Mono,monospace;font-size:12px;cursor:pointer;font-weight:700';
    function submit() {
      var v = inp.value.trim().slice(0, 20);
      if (!v) return;
      localStorage.setItem('arcade_player_name', v);
      wrap.remove();
    ...
    if (joinBtn) joinBtn.parentNode.insertBefore(wrap, joinBtn.nextSibling);
    else { var lbBody = document.querySelector('.gc-lb-body'); if (lbBody) lbBody.appendChild(wrap); }
  ...
  function buildLinkIdentityHtml() {
    if (!window.AuthManager) return '';
    var isAnon = window.AuthManager.isAnonymous();
    var hasAuth = window.AuthManager.hasSession();
    if (hasAuth && !isAnon) {
      return '<div class="gc-link-section"><div class="gc-link-status">🔗 Profile linked — your progress syncs across devices.</div></div>';
    }
    return '<div class="gc-link-section">' +
      '<div class="gc-section-title" style="margin-top:16px">Save Your Progress</div>' +
      '<div class="gc-link-desc">Link your profile to access XP, achievements, and scores on any device.</div>' +
      '<div class="gc-link-actions">' +
        '<button class="gc-link-btn gc-link-email" id="gcLinkEmail">📧 Link Email</button>' +
        '<button class="gc-link-btn gc-link-linkedin" id="gcLinkLinkedIn">🔗 Link LinkedIn</button>' +
      '</div>' +
      '<div class="gc-link-form" id="gcLinkEmailForm" style="display:none">' +
        '<input type="email" class="gc-link-input" id="gcLinkEmailInput" placeholder="your@email.com" maxlength="100">' +
        '<button class="gc-link-submit" id="gcLinkEmailSubmit">Save</button>' +
        '<div class="gc-link-error" id="gcLinkError"></div>' +
      '</div>' +
    '</div>';
  }

  function initLinkIdentityHandlers() {
    var emailBtn = document.getElementById('gcLinkEmail');
    var linkedinBtn = document.getElementById('gcLinkLinkedIn');
    var emailForm = document.getElementById('gcLinkEmailForm');
    if (emailBtn) {
      emailBtn.addEventListener('click', function() {
        if (emailForm) emailForm.style.display = emailForm.style.display === 'none' ? 'block' : 'none';
        var inp = document.getElementById('gcLinkEmailInput');
        if (inp) setTimeout(function() { inp.focus(); }, 100);
      });
    }
    var submitBtn = document.getElementById('gcLinkEmailSubmit');
    if (submitBtn) {
      submitBtn.addEventListener('click', async function() {
        var inp = document.getElementById('gcLinkEmailInput');
        var errEl = document.getElementById('gcLinkError');
        if (!inp || !inp.value.trim()) { if (errEl) errEl.textContent = 'Enter an email address.'; return; }
        submitBtn.disabled = true;
        submitBtn.textContent = 'Linking...';
        if (errEl) errEl.textContent = '';
        var result = await window.AuthManager.linkEmail(inp.value.trim());
        if (result.error) {
          if (errEl) errEl.textContent = result.error.message || 'Link failed.';
          submitBtn.disabled = false;
          submitBtn.textContent = 'Save';
        } else {
          if (window.VDna && window.VDna.syncToCloud) window.VDna.syncToCloud();
          if (window._haptic) window._haptic.success();
          if (window.UniToast) window.UniToast('Profile linked! Check your email to confirm.', '', '🔗', 'success');
          renderTab('progress');
        }
      });
    }
    if (linkedinBtn) {
      linkedinBtn.addEventListener('click', async function() {
        if (!window.AuthManager) return;
        var result = await window.AuthManager.linkOAuth('linkedin_oidc');
        if (result && result.error) {
          if (window.UniToast) window.UniToast('LinkedIn link failed: ' + (result.error.message || 'Unknown error'), '', '⚠️', 'warn');
        }
      });
    }
  }
    ...
    fetchLeaderboard(type, function(data) {
      ...
      if (joinBtn) {
        joinBtn.addEventListener('click', function() {
          promptPlayerName(function() { syncLeaderboard(); loadLeaderboard(type); });
        });
      }
    });
  }
  ...
  function checkExploration() {
    ...
    for (var trophyId in EXPLORE_SECTIONS) {
      ...
      if (!_exploreElCache[trophyId]) _exploreElCache[trophyId] = document.querySelector(EXPLORE_SECTIONS[trophyId]);
      var el = _exploreElCache[trophyId];
      if (!el) continue;
  ...
  function registerTerminalCommands() {
    ...
    T.saveprofile = function() {
      if (!window.AuthManager) return '<span class="term-red">Auth system not loaded.</span>';
      if (window.AuthManager.hasSession() && !window.AuthManager.isAnonymous()) {
        return '<span class="term-green">🔗 Your profile is already linked.</span>';
      }
      setTimeout(function() { openCase('progress'); }, 200);
      return '<span class="term-cyan">🔗 Opening profile linking...</span>';
    };
    T.linkprofile = T.saveprofile;
    T.link = T.saveprofile;
  ...
  function init() {
    ...
    // Periodic challenge check — pauses when tab is hidden
    var _challengeIv = setInterval(checkChallengeProgress, 15000);
    document.addEventListener('visibilitychange', function() {
      if (document.hidden) { clearInterval(_challengeIv); _challengeIv = null; }
      else if (!_challengeIv) { _challengeIv = setInterval(checkChallengeProgress, 15000); }
    });
  }
  ...
  boot();
  console.log('%c🏆 Gamification%c XP + achievements + trophies','background:#f59e0b;color:#06080f;padding:2px 6px;border-radius:3px;font-weight:bold;','color:#8a95a8;padding-left:6px;');
