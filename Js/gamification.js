...
(function GamificationEngine() {
  ...
  var ACHIEVEMENTS = [
    ...
    { id:'theme_cyberpunk',      icon:'🌆', name:'Cyberpunk Activated',     desc:'Enabled the cyberpunk theme',                 cat:'social',  rarity:'common',    xp:10 },
  ...
  var DAILY_POOL = [
    ...
    { id:'d_arcade',     desc:'Play an arcade game',          xp:20, check:function(p){ var a=JSON.parse(localStorage.getItem('arcade_state')||'{}'); return (a.totalPlays||0) > (p._arcadePlaysAtStart||0); } },
    ...
    { id:'d_cyberpunk',  desc:'Enable cyberpunk theme',       xp:15, check:function(p){ return !!(p.unlocked && p.unlocked.theme_cyberpunk); } }
  ];

  var WEEKLY_POOL = [
    { id:'w_3achvs',    desc:'Unlock 3 new achievements',   xp:60, check:function(p){ return (p._weeklyUnlocks||0) >= 3; } },
    { id:'w_allgames',  desc:'Play all 5 arcade games',     xp:65, check:function(p){ var a=JSON.parse(localStorage.getItem('arcade_state')||'{}'); var hs=a.highScores||{}; return Object.keys(hs).length>=4; } },
  ...
  var BRANCH_ACHVS = {
    ...
    hacker:   ['terminal_used','palette_used','theme_cyberpunk','theme_zen','secret',
  ...
  function showAchievementToast(data) {
    ...
    toast.innerHTML = '<div class="toast-shimmer"></div>' +
      '<div class="toast-emoji">' + data.icon + '</div>' +
      '<div class="toast-body">' +
        '<div class="toast-title">Achievement Unlocked!</div>' +
        '<div class="toast-desc">' + data.name + ' — ' + data.desc + '</div>' +
        '<div class="toast-xp">+' + data.xp + ' XP' + comboStr + '</div>' +
        '<div class="toast-rarity" style="color:' + meta.color + '">' + meta.label + '</div>' +
      '</div>';
  ...
  function showLevelUpToast(item) {
    ...
    toast.innerHTML = '<div class="toast-shimmer"></div>' +
      ...
      '<div class="toast-body">' +
        ...
        '<div class="toast-desc">You reached Level ' + item.level + ': ' + info.name + '</div>' +
        '<div class="toast-xp">Keep exploring to unlock more!</div>' +
      '</div>';
    container.appendChild(toast);
    setTimeout(function() { toast.classList.add('show'); }, 50);
    setTimeout(function() { toast.classList.remove('show'); setTimeout(function() { toast.remove(); }, 500); }, 4200);
  ...
  function updateXpUI() {
    ...
    if (xpLvl) {
      var prestige = p.prestige || 0;
      var stars = prestige > 0 ? ' ' + (PRESTIGE_TIERS[Math.min(prestige,3)-1]||PRESTIGE_TIERS[0]).icon : '';
  ...
  function initChallenges() {
    ...
    var arcade = JSON.parse(localStorage.getItem('arcade_state') || '{}');
    ...
    fetchLeaderboard(type, function(data) {
      ...
      data.forEach(function(row, i) {
        ...
        html += '<div class="gc-lb-row' + (isMe ? ' gc-lb-me' : '') + '">' +
          ...
          '<span class="gc-lb-name">' + row.player_name + (prestigeStars ? ' ' + prestigeStars : '') + '</span>' +
          '<span class="gc-lb-score">' + (row[scoreKey] || 0) + (type === 'arcade' ? ' pts' : ' XP') + '</span>' +
          '<span class="gc-lb-lvl">LVL ' + (row.level || 1) + '</span>' +
          '</div>';
      });
      html += '</div>';
  ...
  function init() {
    ...
    // Exploration scroll listener
    window.addEventListener('scroll', function() { requestAnimationFrame(checkExploration); }, { passive: true });
