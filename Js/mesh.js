...
(function DataMesh() {
  ...
  sigChannel.subscribe((status) => {
    if (status === 'SUBSCRIBED') {
      sigReady = true;
      console.log('[Mesh] Signaling channel ready');
      sigFlush();
      retryPendingOffers();
    } else if (status === 'CHANNEL_ERROR') {
      console.warn('[Mesh] Signaling channel error — WebRTC connections will fail');
    }
  });
  ...
  class PeerLink {
    constructor(peerId, isOfferer, onMessage, onStateChange) {
      ...
      this._pc.onnegotiationneeded = async () => {
        ...
        try { await this._doOffer(); } catch (_) {}
        this._negotiating = false;
      };
      ...
      this._sigHandlerOffer = (p) => {
        ...
        if (!isOfferer || this.state === 'open') this._handleOffer(p.sdp);
      };
  ...
  function onPeerState(peerId, state) {
    console.log('[Mesh] Peer', peerId.slice(0, 8), '→', state);
    if (state === 'closed') {
      links.delete(peerId);
  ...
  function connectToPeer(peerId) {
    ...
    console.log('[Mesh] Connecting to peer', peerId.slice(0, 8), isOfferer ? '(offerer)' : '(answerer)', sigReady ? '' : '— signal pending');
  ...
  function retryPendingOffers() {
    for (const [peerId, link] of links) {
      if (link.state === 'connecting' || link.state === 'new') {
        const isOfferer = myId > peerId;
        if (isOfferer) {
          console.log('[Mesh] Retrying offer to', peerId.slice(0, 8));
          link._doOffer();
        }
      }
    }
  ...
  console.log('[Mesh] Data mesh initialized — id:', myId.slice(0, 8));
