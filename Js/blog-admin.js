...
(function() {
    ...
    async function renderCMS(passkeyOnly) {
      ...
      <div class="lb-admin-panel">
        ...
        // Textarea is the immediate editor — always usable from the start
        (function _setupTextareaFallback() {
            if (!editorEl) return;
        ...
        // Try upgrading to CodeMirror 6 in the background (non-blocking)
        (async function _tryUpgradeToCM6() {
    ...
    async function fetchAdminArticles() {
        ...
        try {
            ...
            // ── Auto-publish any articles whose scheduled_at has passed ──
            await _autoPublishDue();
    ...
    /**
     * Auto-publish articles whose scheduled_at <= now but are still published=false.
     * This is the client-side fallback that ensures scheduled articles go live
     * even if the GitHub Actions cron (publish-linkedin.js) hasn't run yet.
     */
    async function _autoPublishDue() {
        if (!window._sb) return;
        var now = new Date();
        var due = _allAdminArticles.filter(function(a) {
            return a.scheduled_at && !a.published && new Date(a.scheduled_at) <= now;
        });
        if (!due.length) return;

        for (var i = 0; i < due.length; i++) {
            try {
                await window._sb.from('longform_articles')
                    .update({ published: true })
                    .eq('id', due[i].id);
                due[i].published = true; // update local cache
                console.log('[blog] Auto-published scheduled article: ' + due[i].title);
            } catch (e) {
                console.warn('[blog] Auto-publish failed for: ' + due[i].id, e);
            }
        }
        if (due.length && window.UniToast) {
            window.UniToast(due.length + ' scheduled article(s) auto-published!', '', '🚀', 'success');
        }
    }

    function _classifyArticle(a) {
        if (a.scheduled_at && !a.published) {
            // Future scheduled → show in scheduled tab
            if (new Date(a.scheduled_at) > new Date()) return 'scheduled';
            // Past scheduled but not yet published → overdue (treat as scheduled so it's visible)
            return 'scheduled';
        }
