...
(function SpatialVoIP() {
  ...
  function createSpatialPipeline(remoteStream) {
    try {
      ...
      console.log('[VoIP] Audio element attached — stream active:', remoteStream.active, 'tracks:', remoteStream.getAudioTracks().length);
  ...
  function onRemoteTrack(peerId, stream, track) {
    console.log('[VoIP] Remote track received from', peerId.slice(0, 8), '— kind:', track.kind, 'readyState:', track.readyState);
    ...
    if (pipeline) {
      ...
      console.log('[VoIP] Spatial audio pipeline created for', peerId.slice(0, 8));
    } else {
  ...
  async function call(peerId) {
    ...
    try {
      ...
      console.log('[VoIP] Call initiated to peer', peerId.slice(0, 8));
  ...
  async function autoAnswer(peerId) {
    console.log('[VoIP] Auto-answering call from', peerId.slice(0, 8));
    if (activeCalls.has(peerId)) { console.log('[VoIP] Already in call, skipping auto-answer'); return; }
    if (!mesh.isConnected(peerId)) { console.warn('[VoIP] Cannot auto-answer — no mesh to', peerId.slice(0, 8)); return; }
    try {
      const stream = await startMic();
      console.log('[VoIP] Mic acquired, adding track to peer', peerId.slice(0, 8));
      const track = stream.getAudioTracks()[0];
      ...
      if (window._haptic) window._haptic.success();
      showCallToast(peerId, 'connected');
      console.log('[VoIP] Auto-answer complete — two-way audio should be active');
    } catch (e) {
  ...
  function handleMeshMessage(peerId, msg) {
    ...
    console.log('[VoIP] Mesh message:', msg.type, 'from', peerId.slice(0, 8));
  ...
  console.log('[VoIP] Spatial VoIP engine initialized');
})();
