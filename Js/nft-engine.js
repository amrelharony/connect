...
(function NFTEngine() {
  ...
  function retroMint() {
    ...
    function mintNext() {
      ...
      lazyMint(id).then(function() { setTimeout(mintNext, 50); }).catch(function() { setTimeout(mintNext, 50); });
    }
    mintNext();
  }
  ...
  function syncToSupabase(entry) {
    ...
    var p = getProfile();
    var doUpsert = function(uid) {
      window._sb.from('nft_lazy_mints').upsert({
        visitor_id: uid || (p.sessionStart ? String(p.sessionStart) : 'anon'),
        user_id: uid || null,
    ...
    if (window.AuthManager) {
      window.AuthManager.getUid().then(doUpsert).catch(function() { doUpsert(null); });
    } else {
      doUpsert(null);
    }
  }
  ...
  var WALLET_DEEPLINKS = [
    { name: 'MetaMask',       icon: '🦊', url: function(u) { return 'https://metamask.app.link/dapp/' + u.replace(/^https?:\/\//, ''); } },
    { name: 'Trust Wallet',   icon: '🛡️', url: function(u) { return 'https://link.trustwallet.com/open_url?coin_id=60&url=' + encodeURIComponent(u); } },
    { name: 'Coinbase Wallet',icon: '🔵', url: function(u) { return 'https://go.cb-w.com/dapp?cb_url=' + encodeURIComponent(u); } }
  ];
  ...
  function getEthereumProvider() {
    ...
    return new Promise(function(resolve) {
      var attempts = 0;
      var maxAttempts = IS_MOBILE ? 5 : 15;
      function check() {
  ...
  function showMobileWalletPicker() {
    var existing = document.getElementById('nftWalletPicker');
    if (existing) existing.remove();

    var overlay = document.createElement('div');
    overlay.id = 'nftWalletPicker';
    overlay.className = 'nft-picker-overlay';
    overlay.addEventListener('click', function(e) { if (e.target === overlay) closeMobileWalletPicker(); });

    var pageUrl = window.location.href;
    var html = '<div class="nft-picker-panel">' +
      '<div class="nft-picker-title">Open in Wallet</div>' +
      '<div class="nft-picker-sub">Select a wallet to connect</div>' +
      '<div class="nft-picker-list">';

    WALLET_DEEPLINKS.forEach(function(w) {
      html += '<a href="' + w.url(pageUrl) + '" class="nft-picker-item" rel="noopener">' +
        '<span class="nft-picker-icon">' + w.icon + '</span>' +
        '<span class="nft-picker-name">' + w.name + '</span>' +
        '<i class="fa-solid fa-arrow-up-right-from-square" style="font-size:10px;opacity:.4;margin-left:auto"></i>' +
      '</a>';
    });

    html += '</div>' +
      '<div class="nft-picker-hint">This opens the site inside the wallet\'s browser where it can connect securely.</div>' +
      '<div class="nft-picker-close" id="nftPickerCancel">Cancel</div>' +
    '</div>';

    overlay.innerHTML = html;
    document.body.appendChild(overlay);
    var cancelBtn = overlay.querySelector('#nftPickerCancel');
    if (cancelBtn) cancelBtn.addEventListener('click', function() { overlay.remove(); });
    requestAnimationFrame(function() { overlay.classList.add('show'); });
  }

  function closeMobileWalletPicker() {
    var el = document.getElementById('nftWalletPicker');
  ...
  var _connectPending = null;

  function isPendingError(err) {
    if (!err) return false;
    var msg = (err.message || '') + (err.info && err.info.error ? err.info.error.message || '' : '');
    return msg.indexOf('-32002') !== -1 || msg.indexOf('already pending') !== -1;
  }

  function friendlyWalletError(err) {
    if (!err) return 'Unknown error';
    if (isPendingError(err)) return null;
    if (err.code === 4001 || err.code === 'ACTION_REJECTED') return null;
    var msg = err.reason || err.shortMessage || '';
    if (!msg && err.info && err.info.error) msg = err.info.error.message || '';
  ...
  function connect() {
    if (_connectPending) return _connectPending;
    showToast('Detecting wallet...', 'Searching for provider', '🔍', 'accent');
    _connectPending = getEthereumProvider().then(function(eth) {
      if (!eth) {
        if (IS_MOBILE) {
          showMobileWalletPicker();
          return Promise.reject(new Error('_silent'));
        }
      ...
      _connectPending = null;
      if (err && err.message === '_silent') throw err;
      if (isPendingError(err)) {
        showToast('Popup Already Open', 'Check for a pending MetaMask popup and approve or reject it', '🦊', 'warn');
      } else {
        var msg = friendlyWalletError(err);
        if (msg) showToast('Connection Failed', msg, '❌', 'warn');
      }
      throw err;
    });
  ...
  function materialize(achievementId) {
    ...
    // Passkey gate: if registered but not verified this session, require biometric
    if (window._passkey && window._passkey.credentialCount > 0 && !window._passkey.isAuthenticated) {
      showToast('Biometric Required', 'Verify your identity before materializing', '🔐', 'accent');
      return window._passkey.authenticate().then(function() {
        showMaterializeModal(entry);
        return entry;
      }).catch(function(err) {
        if (err.name !== 'NotAllowedError') {
          showToast('Auth Failed', 'Passkey verification required for on-chain transactions', '⚠️', 'warn');
        }
        throw err;
      });
    }
  ...
  function showNFTModal(entry, mode) {
    ...
    var isMat = mode === 'materialize';
    var hashShort = entry.tokenHash ? entry.tokenHash.slice(0, 10) + '…' : '';
    ...
    if (isMat) {
      ...
      details =
        '<div class="nft-mat-detail">Earned: ' + new Date(entry.earnedAt).toLocaleDateString() + '</div>' +
        (hashShort ? '<div class="nft-mat-detail">Hash: <span style="font-family:monospace;color:' + rc.main + ';opacity:.7">' + hashShort + '</span></div>' : '') +
        '<div class="nft-mat-detail">Status: ☁️ Off-chain (collected)</div>';
    }

    _matModal.innerHTML =
      '<div class="nft-mat-panel">' +
        '<div class="nft-mat-preview">' +
          '<img src="' + entry.svgDataUri + '" alt="NFT Preview" class="nft-mat-svg" style="border-color:' + rc.border + ';box-shadow:0 0 24px ' + rc.glow + ',0 0 48px ' + rc.glow + '"/>' +
        '</div>' +
        ...
        (isMat
          ...
          : '<button class="nft-mat-btn" id="nftMatBtn">📥 Download SVG</button>') +
        '<div class="nft-mat-cancel" id="nftMatCancel">' + (isMat ? 'Cancel' : 'Close') + '</div>' +
      '</div>';

    document.body.appendChild(_matModal);
    requestAnimationFrame(function() { _matModal.classList.add('show'); });

    var actionBtn = document.getElementById('nftMatBtn');
    if (isMat) {
      actionBtn.addEventListener('click', function() {
        actionBtn.textContent = '⏳ Waiting for wallet...';
        actionBtn.disabled = true;
        actionBtn.classList.add('pending');
        executeMaterialize(entry).catch(function(e){ console.warn('NFT materialize failed:', e); });
      });
    } else {
      actionBtn.addEventListener('click', function() {
        downloadSVG(entry.achievementId);
      });
    }
  ...
  function showPreviewModal(entry) { showNFTModal(entry, 'preview'); }
  function showMaterializeModal(entry) { showNFTModal(entry, 'materialize'); }
  ...
  function updateWalletUI() {
    ...
    if (!btn) return;
    var pkVerified = window._passkey && window._passkey.isAuthenticated;
    if (isConnected()) {
      btn.classList.add('connected');
      btn.title = truncAddr(_walletAddress) + (pkVerified ? ' · Passkey Verified' : '');
      if (dot) { dot.style.display = 'block'; if (pkVerified) dot.style.background = '#22c55e'; }
    } else {
      btn.classList.remove('connected');
      btn.title = pkVerified ? 'Wallet · Passkey Verified' : 'Connect Wallet';
      if (dot) { dot.style.display = pkVerified ? 'block' : 'none'; if (pkVerified) dot.style.background = '#22c55e'; }
    }
  }
  ...
  function renderNFTsTab(body) {
    ...
    var pkAvail = window._passkey && window._passkey.platformReady;
    var pkAuthed = window._passkey && window._passkey.isAuthenticated;
    var pkName = pkAuthed ? window._passkey.getVerifiedName() : null;
    var pkCount = window._passkey ? window._passkey.credentialCount : 0;

    var html = '<div class="nft-tab-header">';

    // Passkey identity card
    if (pkAvail) {
      html += '<div class="nft-pk-card' + (pkAuthed ? ' nft-pk-verified' : '') + '">' +
        '<div class="nft-pk-row">' +
          '<div class="nft-pk-icon"><i class="fa-solid fa-fingerprint"></i></div>' +
          '<div class="nft-pk-info">' +
            '<div class="nft-pk-label">' + (pkAuthed ? '<i class="fa-solid fa-circle-check" style="color:#22c55e;margin-right:4px;font-size:9px"></i>Biometric Verified' : 'Biometric Identity') + '</div>' +
            '<div class="nft-pk-detail">' + (pkAuthed ? escXml(pkName || 'Authenticated') : (pkCount > 0 ? 'Passkey registered — tap to verify' : 'Secure your vault with biometrics')) + '</div>' +
          '</div>' +
          (pkAuthed
            ? '<div class="nft-pk-badge">FIDO2</div>'
            : '<button class="nft-pk-btn" id="nftPkBtn"><i class="fa-solid fa-fingerprint" style="margin-right:4px"></i>' + (pkCount > 0 ? 'Verify' : 'Setup') + '</button>'
          ) +
        '</div>' +
      '</div>';
    }
    ...
    if (isConnected()) {
      html += '<div class="nft-wallet-pill">' +
        ...
        (pkAuthed ? '<span class="nft-pk-wallet-badge"><i class="fa-solid fa-shield-halved" style="font-size:8px"></i></span>' : '') +
    ...
    // Bind events
    var pkBtn = document.getElementById('nftPkBtn');
    if (pkBtn) pkBtn.addEventListener('click', function() {
      if (window._passkey && window._passkey.credentialCount > 0) {
        window._passkey.authenticate().then(function() { renderNFTsTab(body); }).catch(function() {});
      } else if (window._openPasskey) {
        window._openPasskey();
      }
    });
    var connectBtn = document.getElementById('nftConnectBtn');
    if (connectBtn) connectBtn.addEventListener('click', function() { connect().then(function() { renderNFTsTab(body); }).catch(function(){}); });
    ...
    body.querySelectorAll('.nft-card-materialize').forEach(function(btn) {
      btn.addEventListener('click', function() {
        if (!CONTRACT_ADDRESS) {
          var entry = getLazyMintedById(btn.dataset.mid);
          if (entry) showPreviewModal(entry);
        } else {
  ...
  function showToast(title, desc, icon, type) {
    ...
    toast.innerHTML = '<div class="toast-shimmer"></div>' +
      '<div class="toast-emoji">' + escXml(icon || '🔔') + '</div>' +
      '<div class="toast-body">' +
        '<div class="toast-title">' + escXml(title) + '</div>' +
        (desc ? '<div class="toast-desc">' + escXml(desc) + '</div>' : '') +
      '</div>';
  ...
  function registerTerminalCommands() {
    ...
    T.wallet = function(args) {
      var a = (args || '').trim().toLowerCase();
      var pkLine = '';
      if (window._passkey && window._passkey.isAvailable()) {
        var pkAuthed = window._passkey.isAuthenticated;
        var pkName = pkAuthed ? window._passkey.getVerifiedName() : null;
        var pkCount = window._passkey.credentialCount;
        pkLine = '\n<span class="term-white">Passkey:</span>  ' +
          (pkAuthed ? '<span class="term-green">✅ Verified' + (pkName ? ' (' + escXml(pkName) + ')' : '') + '</span>' :
           pkCount > 0 ? '<span class="term-yellow">🔒 Registered (not verified)</span>' :
           '<span class="term-gray">Not configured — type "passkey" to set up</span>');
      }
      if (a === 'connect') {
        if (isConnected()) return '<span class="term-green">Already connected:</span> ' + _walletAddress;
        connect().catch(function(){});
        return '<span class="term-cyan">Connecting Web3 wallet...</span>';
      }
      var out = '<span class="term-cyan">🔐 Wallet Status</span>\n';
      out += '<span class="term-white">Web3:</span>    ' +
        (isConnected() ? '<span class="term-green">Connected — ' + _walletAddress + '</span>' : '<span class="term-gray">Not connected</span>') +
        pkLine + '\n';
      if (!isConnected() || !pkLine) {
        out += '\n<span class="term-gray">Usage:</span>';
        if (!isConnected()) out += '\n  <span class="term-white">wallet connect</span>  Connect MetaMask / Web3 wallet';
        if (isConnected()) out += '\n  <span class="term-white">disconnect</span>      Disconnect wallet';
        if (window._passkey && !window._passkey.isAuthenticated)
          out += '\n  <span class="term-white">passkey</span>         Biometric identity (WebAuthn)';
      } else {
        out += '\n<span class="term-gray">Type "disconnect" to disconnect · "passkey" for biometric options</span>';
      }
      return out;
    };
    T.connect = function() {
  ...
  var _ethListenersSet = false;
  function setupEthListeners() {
    if (_ethListenersSet) return;
    var eth = window.ethereum || (_eip6963Providers.length > 0 ? _eip6963Providers[0].provider : null);
    if (!eth || typeof eth.on !== 'function') return;
    _ethListenersSet = true;
  ...
  function boot() {
    if (window._coreReady && window.VDna) init();
    else window.addEventListener('AmrOS:CoreReady', init, { once: true });
  }
  boot();
  console.log('%c🎨 NFT Engine%c Achievement minting ready','background:#8b5cf6;color:#fff;padding:2px 6px;border-radius:3px;font-weight:bold;','color:#8a95a8;padding-left:6px;');
