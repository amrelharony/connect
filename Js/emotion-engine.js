...
(function EmotionEngine() {
  ...
  const MOOD_COLORS = {
    warm:       { accent: '#fbbf24', accent2: '#f59e0b', accent3: '#ef4444', glow: 'rgba(251,191,36,.35)',  glowS: 'rgba(251,191,36,.18)' },
    focused:    { accent: '#6366f1', accent2: '#818cf8', accent3: '#a78bfa', glow: 'rgba(99,102,241,.35)',  glowS: 'rgba(99,102,241,.18)' },
    surprised:  { accent: '#a855f7', accent2: '#c084fc', accent3: '#e879f9', glow: 'rgba(168,85,247,.35)',  glowS: 'rgba(168,85,247,.18)' },
    calm:       { accent: '#2dd4bf', accent2: '#5eead4', accent3: '#a7f3d0', glow: 'rgba(45,212,191,.35)',  glowS: 'rgba(45,212,191,.18)' },
    melancholy: { accent: '#60a5fa', accent2: '#93c5fd', accent3: '#a5b4fc', glow: 'rgba(96,165,250,.35)', glowS: 'rgba(96,165,250,.18)' },
    serene:     { accent: '#94a3b8', accent2: '#cbd5e1', accent3: '#a5b4fc', glow: 'rgba(148,163,184,.35)', glowS: 'rgba(148,163,184,.18)' },
    intense:    { accent: '#ef4444', accent2: '#f87171', accent3: '#fb923c', glow: 'rgba(239,68,68,.4)',    glowS: 'rgba(239,68,68,.2)' },
    neutral:    { accent: '#8a95a8', accent2: '#94a3b8', accent3: '#a1a1aa', glow: 'rgba(138,149,168,.2)',  glowS: 'rgba(138,149,168,.1)' },
    playful:    { accent: '#f472b6', accent2: '#fb7185', accent3: '#c084fc', glow: 'rgba(244,114,182,.4)', glowS: 'rgba(244,114,182,.2)' },
    determined: { accent: '#f59e0b', accent2: '#fbbf24', accent3: '#fb923c', glow: 'rgba(245,158,11,.4)',  glowS: 'rgba(245,158,11,.2)' },
    curious:    { accent: '#34d399', accent2: '#6ee7b7', accent3: '#5eead4', glow: 'rgba(52,211,153,.35)', glowS: 'rgba(52,211,153,.18)' }
  };
  ...
  // ═══════════════════════════════════════════════════
  function weatherCodeToMood(code, isDay) {
    code = Number(code);
    if (code === 0) return isDay ? 'warm' : 'serene';
    if (code === 1) return isDay ? 'calm' : 'serene';
    if (code === 2) return 'neutral';
    if (code === 3) return 'melancholy';
    if (code <= 48) return 'serene';
    if (code <= 55) return 'melancholy';
    if (code <= 57) return 'determined';
    if (code <= 63) return 'melancholy';
    if (code <= 67) return 'intense';
    if (code <= 73) return 'serene';
    if (code <= 77) return 'curious';
    if (code <= 80) return 'playful';
    if (code <= 82) return 'intense';
    if (code <= 86) return 'surprised';
  ...
  function initWeatherMood() {
    try {
      ...
      if (parsed && parsed.data && parsed.data.code !== undefined) {
        setMood(weatherCodeToMood(parsed.data.code, parsed.data.isDay), 'weather', 1);
      }
    } catch (_) {}
  }

  window._setWeatherMood = function(code, isDay) {
    if (currentSource === 'face') return;
    setMood(weatherCodeToMood(code, isDay), 'weather', 1);
  };
  ...
  var _lastCSSWrite = 0;
  var _cssTimer = null;
  var CSS_MIN_INTERVAL = 500;

  function _writeMoodVars(colors) {
    _lastCSSWrite = Date.now();
    var a2 = colors.accent2 || colors.accent;
    var a3 = colors.accent3 || colors.accent;
    styleEl.textContent =
      ':root{--accent:' + colors.accent + ';--accent2:' + a2 + ';--accent3:' + a3 + ';--glow:' + colors.glow + ';--glowS:' + colors.glowS + '}' +
      '.light-mode{--accent:' + colors.accent + ';--accent2:' + a2 + ';--accent3:' + a3 + ';--glow:' + colors.glow + ';--glowS:' + colors.glowS + '}';
  }
  ...
  function applyMoodCSS(mood) {
    ...
    if (!colors || mood === 'neutral') {
      if (_cssTimer) { clearTimeout(_cssTimer); _cssTimer = null; }
      styleEl.textContent = '';
      return;
    }

    var now = Date.now();
    if (now - _lastCSSWrite < CSS_MIN_INTERVAL) {
      if (_cssTimer) clearTimeout(_cssTimer);
      var c = colors;
      _cssTimer = setTimeout(function() {
        _cssTimer = null;
        _writeMoodVars(c);
      }, CSS_MIN_INTERVAL - (now - _lastCSSWrite));
      return;
    }

    if (_cssTimer) { clearTimeout(_cssTimer); _cssTimer = null; }
    _writeMoodVars(colors);
  }
  ...
  function classify(shapes) {
    ...
    window._lastFaceData = {
      blinkL: eyeBlkL, blinkR: eyeBlkR, jawOpen: jawOpen,
      lookUpL: get('eyeLookUpLeft'), lookUpR: get('eyeLookUpRight'),
      lookDownL: get('eyeLookDownLeft'), lookDownR: get('eyeLookDownRight')
    };
  ...
  document.addEventListener('moodchange', function(e) {
    var d = e.detail;
    if (window._lake) window._lake.logEvent('mood', d.mood + ':' + d.source);
    if (window.VDna) { var p = window.VDna.get(); p._lastMood = d.mood; p._moodSource = d.source; }
  });
  ...
  console.log('%c🎭 Emotion Engine%c Mood: '+(currentMood||'neutral')+' ('+currentSource+')','background:#a855f7;color:#fff;padding:2px 6px;border-radius:3px;font-weight:bold;','color:#8a95a8;padding-left:6px;');
  if(window._registerTeardown) window._registerTeardown(function() { if (window._emotionEngine) window._emotionEngine.destroy(); });
})();
