...
const btn=document.getElementById('vcBtn'),txt=document.getElementById('vcTxt'),icon=btn.querySelector('i');btn.classList.add('vcb-done');icon.className='fa-solid fa-check';txt.textContent='SAVED!';if(window._haptic)window._haptic.success();setTimeout(()=>{btn.classList.remove('vcb-done');icon.className='fa-solid fa-address-card';txt.textContent='Save Contact';},2500);};
...
document.querySelectorAll('.share-plat').forEach(btn=>{
    btn.addEventListener('click',()=>{
        ...
        if(p==='copy'){
            navigator.clipboard.writeText(SHARE_URL).then(()=>{showCopied();if(window._haptic)window._haptic.success();});
        } else if(p==='native'){
        ...
        VDna.addXp(5);
        VDna.get()._shared=true;VDna.save();
    });
});
...
setTimeout(()=>{const btn=document.getElementById('trophyBtn');btn.style.display='flex';btn.addEventListener('click',function(){if(window._game)window._game.openCase('achievements');else if(window.openTrophy)window.openTrophy();});document.getElementById('termBtn').style.display='flex';document.getElementById('gameBtn').style.display='flex';},4000);
...
`;
  ...
  function initZenMode() {
    ...
    var trophyRef = document.getElementById('trophyBtn');
    (trophyRef || document.getElementById('gameBtn')).insertAdjacentElement('afterend', gbBtn);
...
`;
  ...
  function initVoiceNav() {
    ...
    const ROUTES = [
      ...
      { match:/cyberpunk|neon|night\s*city|theme/i,           action:()=>{if(window._toggleCyberpunk)window._toggleCyberpunk(true);if(window._game)window._game.unlock('theme_cyberpunk');return'🌆 Toggling Cyberpunk';} },
...
<span class="term-cyan">System:</span>
  ...
  <span class="term-white">easter</span>            Find hidden commands 😉`;
    ...
    T.cyberpunk = () => { if (window._toggleCyberpunk) window._toggleCyberpunk(true); if (window._game) window._game.unlock('theme_cyberpunk'); return '<span class="term-green">🌆 Toggling Cyberpunk</span>'; };
    T.matrix = () => {
...
<span class="term-gray">Events:</span>     ${s.eventCount} logged`);
  ...
  function getImageViewer() {
    ...
    viewer.innerHTML =
      '<div class="cert-viewer-inner">' +
        ...
        '<div class="cert-viewer-toolbar">' +
          ...
          '<span class="cert-viewer-zoom-info">100%</span>' +
          ...
          '<button class="cv-btn cv-zoom-reset" aria-label="Reset zoom"><i class="fa-solid fa-expand"></i></button>' +
    ...
    var zoomResetBtn = viewer.querySelector('.cv-zoom-reset');
    ...
    function setZoom(z) {
      ...
      zoomInfo.style.opacity = zoom > 1 ? '1' : '.6';
      if (zoom <= 1) { wrap.scrollTop = 0; wrap.scrollLeft = 0; }
    }
    ...
    var swipeX0 = 0;
    wrap.addEventListener('touchstart', function(e) {
      if (e.touches.length === 1 && zoom <= 1) swipeX0 = e.touches[0].clientX;
    }, { passive: true });
    wrap.addEventListener('touchend', function(e) {
      if (zoom > 1 || !swipeX0) return;
