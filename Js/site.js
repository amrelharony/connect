...
(async function(){
    try{
        ...
        const res=await _fetchT('https://date.nager.at/api/v3/PublicHolidays/'+yr+'/EG',null,6000);
        if(res.ok){
...
(async function(){
    const CACHE_KEY='cairoWeather';const TTL=3600000;
    const _wUrl='https://api.open-meteo.com/v1/forecast?latitude=30.0444&longitude=31.2357&current=temperature_2m,weather_code,is_day,wind_speed_10m&timezone=Africa%2FCairo';
    ...
    function render(data){
        ...
        if(!w||!ico||!tmp)return;
        const faIcon=getWeatherIcon(data.code,data.isDay);
        const iconColor=getWeatherColor(data.code,data.temp,data.isDay);
    ...
    try{
        ...
        if(cached){render(cached);}
            ...
            const res=await _fetchT(_wUrl,null,6000);
            if(res.ok){
                const data=parseRes(await res.json());
    ...
    const _weatherIv=setInterval(async()=>{
        ...
        try{
            const res=await _fetchT(_wUrl,null,6000);
            if(res.ok){const data=parseRes(await res.json());localStorage.setItem(CACHE_KEY,JSON.stringify({data,ts:Date.now()}));render(data);}
        }catch(e){}
    },3600000);
})();
...
function _preloaderDone(){const pre=document.getElementById('preloader');gsap.to('#preCtr',{scale:1.3,opacity:0,duration:.3,ease:'power2.in'});gsap.to(['#preLine','#preBar','#preStg'],{opacity:0,duration:.2});pre.style.clipPath='inset(0 0 0% 0)';gsap.to(pre,{clipPath:'inset(0 0 100% 0)',duration:.7,delay:.2,ease:'power4.inOut',onComplete:()=>{pre.style.display='none';}});if(!reducedMotion&&window._dataIntro){setTimeout(()=>{window._dataIntro.init(window._weatherData,launch);},600);}else{setTimeout(launch,500);}}
...
function launch(){
    ...
    gsap.to(app,{opacity:1,duration:.5,delay:.1});setTimeout(updStatus,400);
...
(function(){
  ...
  if(!hname)return;
  var _lpTimer=null,_lpMenu=null,_lpDismiss=null;

  function removeMenu(){
    if(_lpMenu&&_lpMenu.parentNode)_lpMenu.parentNode.removeChild(_lpMenu);
    _lpMenu=null;
    if(_lpDismiss){document.removeEventListener('click',_lpDismiss,true);_lpDismiss=null;}
  }

  function showAdminMenu(x,y){
    removeMenu();
    var m=document.createElement('div');
    m.id='adminLpMenu';
    m.style.cssText='position:fixed;z-index:100020;background:rgba(6,8,15,.96);border:1px solid rgba(0,225,255,.25);border-radius:12px;padding:6px;backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);box-shadow:0 8px 32px rgba(0,0,0,.5);animation:adminLpIn .2s ease';
    var bx=Math.min(x,window.innerWidth-180),by=Math.max(y-80,12);
    m.style.left=bx+'px';m.style.top=by+'px';
    var btnCss='display:block;width:100%;padding:12px 20px;background:none;border:none;color:#e2e8f0;font-family:"JetBrains Mono",monospace;font-size:11px;letter-spacing:.5px;text-align:left;cursor:pointer;border-radius:8px;transition:background .15s,color .15s;white-space:nowrap';
    m.innerHTML='<button id="alpBlog" style="'+btnCss+'">✍ Blog CMS</button><button id="alpMicro" style="'+btnCss+'">💭 Thoughts</button>';
    document.body.appendChild(m);
    _lpMenu=m;

    document.getElementById('alpBlog').addEventListener('click',function(e){e.stopPropagation();removeMenu();if(window.openBlogAdmin)window.openBlogAdmin();});
    document.getElementById('alpMicro').addEventListener('click',function(e){e.stopPropagation();removeMenu();if(window.openMicroblog)window.openMicroblog();});
    ['alpBlog','alpMicro'].forEach(function(id){
      var b=document.getElementById(id);
      if(b){b.addEventListener('pointerenter',function(){this.style.background='rgba(0,225,255,.1)';this.style.color='#00e1ff';});
      b.addEventListener('pointerleave',function(){this.style.background='none';this.style.color='#e2e8f0';});}
    });

    setTimeout(function(){
      _lpDismiss=function(ev){if(_lpMenu&&!_lpMenu.contains(ev.target)){removeMenu();}};
      document.addEventListener('click',_lpDismiss,true);
    },50);
  ...
  hname.addEventListener('touchend',function(e){
    ...
    if(now-_lastTapTime<350){
      e.preventDefault();
      var rect=hname.getBoundingClientRect();
      if(window._haptic)window._haptic.levelUp();
      showAdminMenu(rect.left+rect.width/2,rect.top+rect.height);
    }
    _lastTapTime=now;
  });
  ...
  hname.addEventListener('contextmenu',function(e){if(_lpMenu)e.preventDefault();});
  ...
  sty.textContent='@keyframes adminLpIn{from{opacity:0;transform:scale(.9) translateY(6px)}to{opacity:1;transform:scale(1) translateY(0)}}';
...
(function ContentHubTabs() {
  ...
  tabCSS.textContent = '.content-hub-tabs{display:flex;justify-content:center;gap:0;margin-bottom:16px}' +
    '.ch-tab{padding:8px 24px;background:none;border:1px solid rgba(0,225,255,.1);border-bottom:2px solid transparent;' +
    "font-family:'JetBrains Mono',monospace;font-size:9px;letter-spacing:1px;text-transform:uppercase;color:#6b7a90;" +
    ...
    '.ch-action-btn{cursor:pointer;border:1px solid rgba(0,225,255,.15);background:rgba(255,255,255,.04);' +
    "backdrop-filter:blur(16px) saturate(180%);-webkit-backdrop-filter:blur(16px) saturate(180%);" +
    "border-radius:14px;padding:10px 28px;font-family:'JetBrains Mono',monospace;font-size:9px;" +
    'color:#8b949e;letter-spacing:1px;text-transform:uppercase;transition:all .3s ease;' +
    'box-shadow:0 4px 20px rgba(0,0,0,.15),inset 0 1px 0 rgba(255,255,255,.06)}' +
    '.ch-action-btn:hover{border-color:rgba(0,225,255,.35);color:#00e1ff;background:rgba(0,225,255,.06);' +
    'transform:translateY(-2px);box-shadow:0 8px 32px rgba(0,225,255,.12),inset 0 1px 0 rgba(255,255,255,.08)}' +
    '.ch-action-btn-alt:hover{border-color:rgba(168,85,247,.35);color:#a855f7;background:rgba(168,85,247,.06);' +
    'box-shadow:0 8px 32px rgba(168,85,247,.12),inset 0 1px 0 rgba(255,255,255,.08)}' +
    '.light-mode .ch-action-btn{border-color:rgba(0,102,255,.12);background:rgba(255,255,255,.6);color:#64748b;' +
    'box-shadow:0 2px 12px rgba(0,0,0,.06)}' +
    '.light-mode .ch-action-btn:hover{border-color:rgba(0,102,255,.3);color:#0066ff;background:rgba(0,102,255,.04);' +
    'box-shadow:0 6px 24px rgba(0,102,255,.1)}' +
    '.light-mode .ch-action-btn-alt:hover{border-color:rgba(124,58,237,.3);color:#7c3aed;background:rgba(124,58,237,.04);' +
  ...
  document.querySelectorAll('.ch-tab').forEach(function(tab) {
    tab.addEventListener('click', function() {
      ...
      tab.classList.add('active');
      document.querySelectorAll('.ch-feed').forEach(function(f) { f.style.display = 'none'; });
...
<span class="term-cyan"> /_/        \\_\\ </span>  <span class="term-cyan">AI</span>: ${window._llmReady ? '<span class="term-green">Sentient</span>' : '<span class="term-gray">Keyword</span>'}`,
    ...
    help: function(args) {
        ...
        for (const cat of ['SYSTEM','FILES','VIEW','NAV','APPS','AI','LINKS','SOCIAL']) {
    ...
    weather: () => {
        ...
        const d=w.data||{};
        const temp=typeof d.temp==='number'?Math.round(d.temp)+'°C':'--°C';
        const codeNames={'0':'Clear sky','1':'Mainly clear','2':'Partly cloudy','3':'Overcast','45':'Fog','48':'Rime fog','51':'Light drizzle','53':'Drizzle','55':'Heavy drizzle','56':'Freezing drizzle','57':'Heavy freezing drizzle','61':'Light rain','63':'Rain','65':'Heavy rain','66':'Freezing rain','67':'Heavy freezing rain','71':'Light snow','73':'Snow','75':'Heavy snow','77':'Snow grains','80':'Light showers','81':'Showers','82':'Heavy showers','85':'Snow showers','86':'Heavy snow showers','95':'Thunderstorm','96':'Thunderstorm + hail','99':'Heavy thunderstorm + hail'};
        const sky=codeNames[String(d.code)]||'Unknown';
        const mood=window._emotionEngine?window._emotionEngine.getMood():{mood:'--',source:'--'};
        const moodStr=mood.source==='weather'?mood.mood:'--';
    ...
    whoami: () => `<span class="term-cyan">Amr Elharony</span>
...
<span class="term-gray">Cairo, Egypt · Banque Misr</span>`,
    ...
    resume: () => `<span class="term-cyan">═ Resume Summary ═</span>
...
<span class="term-gray">Type "linkedin" or "calendar" to connect</span>`,
    ...
    stack: () => `<span class="term-cyan">═ Tech Stack ═</span>
...
<span class="term-green">Web:</span> HTML/CSS/JS · React · Node.js`,
    ...
    scores: () => {
        if (window._game) { setTimeout(() => window._game.openCase('achievements'), 200); return '<span class="term-green">🏆 Opening Trophy Case...</span>'; }
        return '<span class="term-gray">No scores yet — play some arcade games!</span>';
    },

    goto: (args) => {
        const sec = (args || '').trim().toLowerCase();
        const selMap = { timeline: '.tl-wrap', certs: '#certGrid', testimonials: '.tc-section', conferences: '.conf-strip', articles: '#linkedinFeed', impact: '.imp', hero: '.pf', contact: '#contactSecret' };
        if (!sec) return '<span class="term-gray">Usage: goto &lt;section&gt;</span>\n<span class="term-gray">Sections: ' + Object.keys(selMap).join(', ') + '</span>';
        const sel = selMap[sec];
        ...
        if (el) { setTimeout(() => { window.closeTerm(); el.scrollIntoView({ behavior: 'smooth', block: 'start' }); }, 200); }
        return `<span class="term-green">→ Scrolling to ${sec}...</span>`;
    },

    top: () => { setTimeout(() => { window.closeTerm(); window.scrollTo({ top: 0, behavior: 'smooth' }); }, 200); return '<span class="term-green">↑ Scrolling to top...</span>'; },

    bottom: () => { setTimeout(() => { window.closeTerm(); window.scrollTo({ top: document.documentElement.scrollHeight, behavior: 'smooth' }); }, 200); return '<span class="term-green">↓ Scrolling to bottom...</span>'; },

    linkedin: () => { window.open('https://www.linkedin.com/in/amrmelharony', '_blank'); return '<span class="term-green">🔗 Opening LinkedIn...</span>'; },

    calendar: () => { window.open('https://adplist.org/mentors/amr-elharony', '_blank'); return '<span class="term-green">📅 Opening booking page...</span>'; },

    book: () => { window.open('https://bilingualexecutive.amrelharony.com/', '_blank'); return '<span class="term-green">📘 Opening The Bilingual Executive...</span>'; },

    mentor: () => { window.open('https://adplist.org/mentors/amr-elharony', '_blank'); return '<span class="term-green">🎓 Opening ADPList mentoring...</span>'; },

    community: () => { window.open('https://www.linkedin.com/company/fintech-bilinguals/', '_blank'); return '<span class="term-green">🤝 Opening Fintech Bilinguals...</span>'; },
...
function _termUpdateStatus() {
    ...
    if (tsAI) {
        ...
        const cls = isAI ? 'ai' : 'kb';
        const txt = isAI ? 'AI' : 'KB';
    ...
    if (e.key === 'Enter') {
        ...
        if (typeof TermCmds[cmd] === 'function') {
            ...
            if (out === '__CLEAR__') {
                ...
                bootLines.forEach(function(t) { body.innerHTML += '<div class="term-line">' + t + '</div>'; });
            }
...
function updateUI(){
    ...
    dots.querySelectorAll('.tc-dot').forEach(d=>{d.addEventListener('click',()=>{currentIdx=parseInt(d.dataset.idx);updateUI();startAuto();if(window._haptic)window._haptic.tap();});});
...
(function(){
    const unifiedKeys=e=>{
        ...
        if(e.key==='`'||code==='Backquote'){openTerm();setTimeout(function(){var i=document.getElementById('termInput');if(i){i.value='ask ';i.focus();i.setSelectionRange(4,4);}},400);}
...
function loadAI3D(){
  ...
  ai3dLoadPromise=new Promise((resolve,reject)=>{
    var loadMain=function(){
      var s=document.createElement('script');s.src='Js/ai3d.js';
      s.onload=function(){ai3dLoaded=true;resolve();};
      s.onerror=function(){ai3dLoadPromise=null;if(window.UniToast)window.UniToast('Failed to load AI/3D module — retrying...');reject();};
      document.head.appendChild(s);
    };
    var kb=document.createElement('script');kb.src='Js/kb-docs.js';
    kb.onload=kb.onerror=loadMain;
...
const _stubLlm=function(args){loadAI3D().then(()=>{if(window.TermCmds.llm!==_stubLlm){var o=window.TermCmds.llm(args);_termAppend(o);}}).catch(_noop);return '<span class="term-green">Loading AI...</span>';};
window.TermCmds.llm=_stubLlm;
const _stubLlmLoad=function(args){loadAI3D().then(()=>{if(window.TermCmds['llm-load']!==_stubLlmLoad){var o=window.TermCmds['llm-load'](args);_termAppend(o);}}).catch(_noop);return '<span class="term-green">Loading AI...</span>';};
window.TermCmds['llm-load']=_stubLlmLoad;
const _stubLlmOff=function(args){loadAI3D().then(()=>{if(window.TermCmds['llm-off']!==_stubLlmOff){var o=window.TermCmds['llm-off'](args);_termAppend(o);}}).catch(_noop);return '<span class="term-green">Loading AI...</span>';};
window.TermCmds['llm-off']=_stubLlmOff;
const _stubLlmOn=function(args){loadAI3D().then(()=>{if(window.TermCmds['llm-on']!==_stubLlmOn){var o=window.TermCmds['llm-on'](args);_termAppend(o);}}).catch(_noop);return '<span class="term-green">Loading AI...</span>';};
window.TermCmds['llm-on']=_stubLlmOn;
...
window.TermCmds.intro=function(args){
  ...
  var phase=phases.includes(parts[0])?parts[0]:null;
  var atmo=atmos.includes(parts[1])?parts[1]:null;
...
`;
  ...
  function initGuestbook() {
    ...
    document.getElementById('gbGlobeBtn').addEventListener('click', function(){
      ...
      if(typeof loadAI3D!=='function'){if(window.UniToast)window.UniToast('Globe unavailable');return;}
      btn.textContent='Loading...';btn.disabled=true;
      loadAI3D().then(function(){
