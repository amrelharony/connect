...
const SITE = 'https://www.amrelharony.com';
...
async function publishThoughts() {
  ...
  const rows = await sbQuery('microblog',
    `select=id,content,image_url&scheduled_at=not.is.null&scheduled_at=lte.${now}&linkedin_posted=eq.false`
  );
  ...
  for (const row of rows) {
    try {
      ...
      await sbUpdate('microblog', row.id, { linkedin_posted: true });
...
async function publishArticles() {
  ...
  const rows = await sbQuery('longform_articles',
    `select=id,title,slug,excerpt,content&scheduled_at=not.is.null&scheduled_at=lte.${now}&linkedin_posted=eq.false`
  );
