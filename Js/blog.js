...
@media(max-width:768px){
  ...
  .lb-cms-split{grid-template-columns:1fr;min-height:auto;border:none}
  .lb-cms-textarea{border:1px solid var(--border);min-height:220px;border-radius:4px}
  .lb-cms-preview{display:none}
  .lb-cms-row{grid-template-columns:1fr;gap:10px}
  .lb-admin-panel{margin:0;padding:16px;border:none;max-width:100%;min-height:100vh;box-sizing:border-box}
  .lb-admin-panel.lb-auth-panel{min-height:auto;margin:60px 16px 20px;border:1px solid var(--accent)}
  .lb-admin-header{flex-wrap:wrap;gap:8px}
  .lb-article-footer{flex-direction:column;align-items:flex-start}
  #blogAdmin{padding:env(safe-area-inset-top) env(safe-area-inset-right) env(safe-area-inset-bottom) env(safe-area-inset-left)}
  .lb-toolbar{overflow-x:auto;flex-wrap:nowrap;-webkit-overflow-scrolling:touch;padding:6px;gap:1px}
  .lb-toolbar-group{flex-shrink:0}
  .lb-toolbar-group+.lb-toolbar-group{margin-left:2px;padding-left:4px}
  .lb-tool-btn{padding:8px 10px;min-width:32px;font-size:12px}
  .lb-cms-actions{flex-direction:column;gap:10px;align-items:stretch}
  .lb-cms-btn-group{flex-direction:column;gap:8px}
  .lb-cms-btn{flex:1;width:100%;text-align:center;padding:12px 20px;font-size:11px}
  .lb-schedule-row{flex-direction:column;align-items:stretch;gap:8px}
  .lb-schedule-input{width:100%;padding:10px 12px}
  .lb-cms-input{padding:12px 14px;font-size:13px}
  .lb-drafts-header{flex-wrap:wrap;gap:6px}
  .lb-drafts-search{min-width:0}
  .lb-draft-row{flex-wrap:wrap;gap:6px;padding:12px}
  .lb-draft-meta{flex-wrap:wrap}
  .lb-cms-article-row{flex-wrap:wrap;gap:6px;padding:12px 0}
  .lb-cms-article-actions{margin-left:0;width:100%}
  .lb-cms-article-actions .lb-cms-btn{padding:8px 12px}
  .lb-sub-tab{font-size:9px;padding:10px 8px;text-align:center}
  .lb-quick-row{gap:6px}
  .lb-quick-textarea{min-height:120px;font-size:14px;padding:14px}
}
@media(max-width:480px){
  ...
  .lb-admin-panel{padding:12px}
  .lb-admin-panel.lb-auth-panel{margin:40px 8px 14px}
  .lb-cms-textarea{min-height:180px;font-size:13px}
  .lb-admin-title{font-size:10px;letter-spacing:1px}
  .lb-admin-close{padding:10px 16px;font-size:11px}
  .lb-cms-tab{padding:10px 10px;font-size:9px;letter-spacing:.5px}
  .lb-cms-label{font-size:9px}
  .lb-tool-btn{padding:7px 8px;font-size:11px;min-width:28px}
  .lb-sub-tab{font-size:8px;padding:9px 6px;letter-spacing:.4px}
}
...
`;
      ...
      <div class="lb-admin-panel">
        ...
        async function submitQuickPost(scheduledAt) {
            ...
            try {
                ...
                var liShare = scheduledAt && liCheck ? liCheck.checked : false;
        ...
        function updateTDraftsBadge() {
            ...
            if (!badge) return;
            var count = getAllThoughtDrafts().length;
        ...
        // Save Draft button
        document.getElementById('lbQuickDraft').addEventListener('click', function() {
            saveCurrentThoughtDraft(false);
            document.getElementById('lbQuickStatus').textContent = 'Draft saved!';
        ...
        document.getElementById('lbDraftsSearch').addEventListener('input', debounce(function(e) {
            renderDraftsList(e.target.value.trim());
        }, 200));
    ...
    async function saveArticle(publish, scheduledAt) {
        ...
        var row = {
            ...
            linkedin_posted: scheduledAt ? !liShare : true
        };
