...
(function SpatialAudioModule() {
  ...
  const INTERACTIVE = 'button,.tbtn,.lk,.mg-btn,.arcade-card:not(.locked),.always-cta,.gb-emoji-btn,.gb-submit,.swipe-opt,.trophy-btn,.vcb,.term-chat-topic,.sc-row,.si,.conf-badge,.cert-card,.nl,.tl-item,.rt,.share-dl-btn,.tc-cat-btn,.gc-achv,.nft-card,.mp-avatar,.mp-ctx-item,.swipe-card,.pe-cursor,.challenge-hud,.gc-challenge,.sk-era';
  ...
  var sectionTones = {
    ...
    'secSkills':      function() { blip(311, 0.1, -0.2, 'triangle', 0.05); setTimeout(function(){ blip(370, 0.1, 0, 'triangle', 0.05); }, 85); setTimeout(function(){ blip(440, 0.12, 0.2, 'triangle', 0.04); }, 170); }
  };
  ...
  setTimeout(function() {
    ['hname', 'pfw', 'rtags', 'contactSecret', 'secJourney', 'secSkills', 'secCerts', 'secTestimonials', 'secNewsletters', 'secContentHub'].forEach(function(id) {
