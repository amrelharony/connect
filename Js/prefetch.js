...
(function() {
  ...
  const STATE_TO_SCRIPT = {
    terminal: 'Js/chatbot-llm.js', arcade: 'Js/arcade.js', book3d: 'Js/book-ar-viewer.js',
    spatial: 'Js/spatial-controller.js', globe: 'Js/visualizer-globe.js',
    emotion: 'Js/emotion-engine.js'
  };

  const TRIGGER_ELEMENTS = [
    { sel: '#termBtn, [onclick*="openTerm"]', script: 'Js/ai3d-core.js' },
    { sel: '#badge3dPreview', script: 'Js/book-ar-viewer.js' },
    { sel: '#spatialBtn', script: 'Js/spatial-controller.js' },
    { sel: '.gb-globe-btn', script: 'Js/visualizer-globe.js' },
  ...
  // ═══════════════════════════════════════════════════
  // MARKOV TRANSITION MODEL (math in Js/lib/markov-chain.js)
  // ═══════════════════════════════════════════════════
  const STORAGE_KEY = '_prefetchMarkov';
  const MC = window.MarkovChain;
  const _mc = MC.create(N);

  const markov = {
    get matrix() { return _mc.matrix; },
    get totalTransitions() { return _mc.totalTransitions; },

    load() {
      try {
        const raw = localStorage.getItem(STORAGE_KEY);
        if (raw) {
          const parsed = MC.deserialize(raw, N);
          _mc.matrix.set(parsed.matrix);
          _mc.totalTransitions = parsed.totalTransitions;
        }
      } catch (_) {}
      if (_mc.totalTransitions < 20) MC.applyColdPrior(_mc, COLD_PRIOR);
    },

    save() {
      const data = MC.serialize(_mc);
    ...
    transition(from, to) {
      ...
      if (fi < 0 || ti < 0) return;
      MC.transition(_mc, fi, ti);
      if (_mc.totalTransitions % 10 === 0) this.save();
    },

    predict(stateIdx) {
      const preds = MC.predict(_mc, stateIdx, CONFIDENCE_THRESHOLD, 3);
      return preds.map(p => ({ idx: p.idx, state: STATES[p.idx], prob: p.prob }));
    }
  };
  ...
  function initWeightsFromMarkov() {
    const w = MC.initWeightsFromMarkov(_mc, 6, 8);
    W1.set(w.W1); B1.set(w.B1); W2.set(w.W2); B2.set(w.B2);
  }
