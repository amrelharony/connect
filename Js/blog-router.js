...
(function() {
    ...
    function showPortfolio() {
        ...
        if (ww) {
            ww.style.display = '';
            if (window._peripheralsShown) ww.classList.add('show');
        }
        ...
        if (!forceCSS) {
            ...
            forceCSS.textContent = '#app{display:block!important;opacity:1!important}' +
                '.rv,.sa,.nd,.st{opacity:1!important;transform:none!important}' +
        ...
        if (window._updStatus) window._updStatus();
        if (window._showPeripherals) window._showPeripherals();
    ...
    document.addEventListener('keydown', function(e) {
        ...
        if (window._hideArticleToolbar && window._hideArticleToolbar()) { e.preventDefault(); e.stopPropagation(); return; }
        e.preventDefault();
        e.stopPropagation();
        var route = getRoute();
