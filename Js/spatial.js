...
(function SpatialNav() {
  ...
  function initWorker() {
    return new Promise((resolve, reject) => {
      try { worker = new Worker('Js/spatial-worker.js'); }
  ...
  // ── Expose globals (only if spatial-controller.js hasn't already set them) ──
  if (!window._toggleSpatialNav) window._toggleSpatialNav = toggle;
  if (!window._closeSpatialNav)  window._closeSpatialNav  = deactivate;
  if (!window._isSpatialActive)  window._isSpatialActive  = () => active;
})();
