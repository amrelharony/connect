...
(function() {
    ...
    function _sidebarDocClick(e) {
        if (!B._sidebarOpen || !_sidebarEl) return;
        if (_sidebarEl.contains(e.target)) return;
        if (_sidebarToggle && _sidebarToggle.contains(e.target)) return;
        _closeSidebar();
    }

    function _sidebarEscKey(e) {
        if (e.key === 'Escape' && B._sidebarOpen) {
            e.preventDefault();
            e.stopPropagation();
            _closeSidebar();
        }
    ...
    function _createSidebar() {
        ...
        _sidebarEl.innerHTML =
            ...
            '<ul class="lb-sidebar-nav">' +
                ...
                '<li class="lb-sidebar-item"><button class="lb-sidebar-link" data-action="zen"><i class="fa-solid fa-eye"></i> Zen Mode</button></li>' +
                '<li class="lb-sidebar-item"><button class="lb-sidebar-link" data-action="audio"><i class="fa-solid fa-volume-high" id="lbSidebarAudioIcon"></i> Sound</button></li>' +
                '<li class="lb-sidebar-item"><button class="lb-sidebar-link" data-action="voice"><i class="fa-solid fa-microphone" id="lbSidebarVoiceIcon"></i> Voice</button></li>' +
                ...
                '<li class="lb-sidebar-item"><button class="lb-sidebar-link" data-action="guestbook"><i class="fa-solid fa-book"></i> Guestbook</button></li>' +
        ...
        document.addEventListener('pointerdown', _sidebarDocClick);
        document.addEventListener('keydown', _sidebarEscKey, true);
        ...
        _sidebarEl.querySelectorAll('.lb-sidebar-link[data-action]').forEach(function(btn) {
            btn.addEventListener('click', function() {
                ...
                if (action === 'rss') {
                    ...
                    else if (window._spatialAudio) { var on = window._spatialAudio.toggle(); if (window._syncAudioBtn) window._syncAudioBtn(on); }
                    var saIcon = document.getElementById('lbSidebarAudioIcon');
                    if (saIcon) {
                        var isOn = audioBtn && audioBtn.classList.contains('active');
                        saIcon.className = isOn ? 'fa-solid fa-volume-high' : 'fa-solid fa-volume-xmark';
                    }
