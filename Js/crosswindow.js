...
(function() {
  ...
  bc.onmessage = function(e) {
    var m = e.data;
    if (m.from === ID) return;
    if (m.type === 'geo')   { peers[m.from] = { geo: m.geo, seen: Date.now(), tearoff: m.tearoff }; return; }
  ...
  function pollChild(k) {
    var iv = setInterval(function() {
      if (!children[k] || children[k].closed) { delete children[k]; clearInterval(iv); }
    }, 600);
  }
