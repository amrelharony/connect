...
(function() {
  ...
  function processCmd(m) {
    ...
    else                   { send({ type: 'result', html: typeof DOMPurify !== 'undefined' ? DOMPurify.sanitize(r) : r }); }
  }
  ...
  function termScript() {
    return [
      ...
      '    d.innerHTML=typeof DOMPurify!=="undefined"?DOMPurify.sanitize(m.html):m.html;body.appendChild(d);body.scrollTop=body.scrollHeight;',
      '  }',
  ...
  function frame(now) {
    ...
    else { _cwRafRunning = false; return; }
  ...
  console.log('%c🪟 Cross-Window%c DOM teleportation ready','background:#14b8a6;color:#fff;padding:2px 6px;border-radius:3px;font-weight:bold;','color:#8a95a8;padding-left:6px;');
