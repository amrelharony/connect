...
(function() {
    ...
    async function renderFeed() {
      ...
      <div class="lb-wrap" style="max-width:900px">
        ...
        // Auto-publish any scheduled articles that are past due (non-blocking)
        _autoPublishScheduledArticles();
    ...
    /**
     * Non-blocking auto-publish: check for articles where scheduled_at <= now
     * but published is still false, and flip them to published.
     * This is a fire-and-forget background task that ensures scheduled articles
     * go live even without the GitHub Actions cron job.
     */
    async function _autoPublishScheduledArticles() {
        ...
        try {
            ...
            for (var i = 0; i < r.data.length; i++) {
                await window._sb.from('longform_articles')
                    ...
                    .eq('id', r.data[i].id);
                console.log('[blog] Auto-published scheduled article: ' + r.data[i].title);
            }

            // Invalidate feed cache so the next render picks up the newly published article(s)
            B._fetchCache.delete('feed');
        } catch (e) {
