...
`; document.head.appendChild(css);
  ...
  const isAdmin = () => !!(window._passkey && window._passkey.isAuthenticated);
...
<span class="blog-sr-only" id="blogAnnounce" aria-live="assertive"></span>`;
  ...
  async function createPost() {
    ...
    try {
      let img = null;
      if (pendFile) { const ext = pendFile.name.split('.').pop() || 'jpg'; const fn = Date.now() + '_' + Math.random().toString(36).slice(2, 8) + '.' + ext; const { data: ud, error: ue } = await window._sb.storage.from(BUCKET).upload(fn, pendFile, { cacheControl: '3600', upsert: false }); if (ue) throw ue; const { data: urlD } = window._sb.storage.from(BUCKET).getPublicUrl(ud.path); img = urlD.publicUrl; }
      const row = { content: c || '' }; if (img) row.image_url = img;
      // Detect poll syntax: [poll:Question?|Option1|Option2|Option3]
      const pollMatch = c.match(/\[poll:(.+?)\|(.+?)\]/); if (pollMatch) { const parts = pollMatch[2].split('|'); row.poll = { question: pollMatch[1], options: parts.map(o => ({ text: o.trim(), votes: 0 })) }; row.content = c.replace(/\[poll:.+?\]/, '').trim() || pollMatch[1]; }
      const { error } = await window._sb.from('microblog').insert(row); if (error) throw error;
  ...
  /* ── REACTIONS ── */
  async function toggleRx(pid, rk) { if (!window._sb) return; const ur = getJ(REACTIONS_KEY, {}); const my = ur[pid] || []; const was = my.includes(rk); if (was) ur[pid] = my.filter(k => k !== rk); else { if (!ur[pid]) ur[pid] = []; ur[pid].push(rk); } setJ(REACTIONS_KEY, ur); snd('tap'); try { const { data } = await window._sb.from('microblog').select('reactions').eq('id', pid).single(); if (data) { const rx = data.reactions || {}; rx[rk] = Math.max(0, (rx[rk] || 0) + (was ? -1 : 1)); await window._sb.from('microblog').update({ reactions: rx }).eq('id', pid); const p = posts.find(x => x.id === pid); if (p) { p.reactions = rx; render(posts); } } } catch (e) { } }
  ...
  function trackView(pid) {
    ...
    window._sb.rpc('increment_views', { post_id: pid }).catch(() => { });
  }
  ...
  function render(ps) {
    ...
    let html = list.map((p, i) => {
      ...
      let adm = ''; if (admin) adm = `<button class="blog-action-btn" data-edit="${p.id}" title="Edit"><i class="fa-solid fa-pen"></i></button><button class="blog-action-btn" data-pin="${p.id}" title="${p.pinned ? 'Unpin' : 'Pin'}"><i class="fa-solid fa-thumbtack"></i></button><button class="blog-action-btn delete-btn" data-del="${p.id}" title="Delete"><i class="fa-solid fa-trash-can"></i></button>`;
  ...
  async function votePoll(pid, idx) {
    ...
    if (window._sb) { try { await window._sb.from('microblog').update({ poll: p.poll }).eq('id', pid); } catch (e) { } }
    render(posts);
  }
  ...
  document.addEventListener('keydown', e => {
    ...
    if ((e.key === 'b' || e.key === 'B') && !document.querySelector('#termOverlay.show,#gameOverlay.show,#trophyOverlay.show,#shareOverlay.show,#shortcutOverlay.show')) { if (ov.classList.contains('show')) closeBlog(); else openBlog(); return; }
