...
async function insertScore(game, score, player) {
  const p = player || 'Anon';
  const existing = await execSQL("SELECT score FROM scores WHERE game=?1 AND player=?2", [game, p]);
  if (existing.rows && existing.rows.length > 0 && existing.rows[0].score >= score) return;
  await execSQL(
