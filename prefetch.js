...
(function() {
  ...
  const STATE_TO_SCRIPT = {
    terminal: 'ai3d.js', arcade: 'arcade.js', book3d: 'ai3d.js',
    spatial: 'spatial-controller.js', globe: 'ai3d.js'
  };

  const TRIGGER_ELEMENTS = [
    { sel: '#termBtn, [onclick*="openTerm"]', script: 'ai3d.js' },
    { sel: '#badge3dPreview', script: 'ai3d.js' },
    { sel: '#spatialBtn', script: 'spatial-controller.js' },
    { sel: '.gb-globe-btn', script: 'ai3d.js' },
    { sel: '[onclick*="openGame"], [onclick*="openArcade"]', script: 'arcade.js' },
  ];
  ...
  function doPrefetch(scriptUrl) {
    if (!scriptUrl || prefetched.has(scriptUrl) || activePrefetches >= MAX_PREFETCH) return;
  ...
  async function predictionLoop() {
    if (destroyed) return;
    const now = Date.now();
    if (now - lastPredictTime < PREDICT_HZ) {
